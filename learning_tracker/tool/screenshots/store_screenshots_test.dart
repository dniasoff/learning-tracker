/// Manual Play Store screenshot generator — NOT a regression test.
///
/// AUD-t-cross-33: this file previously lived at
/// `test/golden/store_screenshots_test.dart`, where `flutter test` /
/// `make ci` / the coverage floor picked it up as if it were regression
/// coverage. It has been moved out of `test/` because Screenshots 3-5
/// ("Progress Overview", "Scheduler", "Achievements / Gamification") render
/// entirely hand-built mockup widgets defined at the bottom of this file
/// (`_StatCard`, `_WeeklyChart`, `_CurriculumProgress`, `_MilestoneItem`,
/// `_TaskItem`, `_SummaryRow`, `_AchievementTile`) with hardcoded marketing
/// copy ('2,847', '47 days left', a fixed achievement list) — this file
/// imports no widget from the production progress screen, scheduler
/// screen, or achievements screen, so a real UI regression in those
/// screens cannot make Screenshots 3-5 fail; `matchesGoldenFile` there only
/// confirms this file's own mockups still match their own previous
/// mockups. (Screenshots 1-2 do render real production widgets —
/// `StreakWidget`, `ProfileAvatar`, `AppBarTitle` — but stay in this same
/// file so the five Play Store screenshots are still generated by one
/// command; see docs/deployment-guide.md §6.)
///
/// To regenerate the PNGs in `goldens/` after a redesign, run from
/// `learning_tracker/`:
...
void main() {
  ...
  setUp(() {
    ...
    //
    // analyzer's `invalid_use_of_visible_for_testing_member` check
    // (package:analyzer PubWorkspacePackage.isInTestDirectory) allows
    // @visibleForTesting members only for files inside the package's
    // literal `test/` directory — a path heuristic, not a "is this a test
    // file" check. AUD-t-cross-33 deliberately moved this file to
    // tool/screenshots/ (see the file-level doc comment) because it is a
    // manual asset generator, not `flutter test`/`make ci` regression
    // coverage; it still legitimately needs this testing-only shim to
    // render the real production widgets used in Screenshots 1-2.
