...
/// Domain-level completion record.
///
/// Previously auto-generated by Drift from the `completions` table.
/// The `completions` table was dropped in W3.20 — this class is now
/// hand-written and maps 1:1 to [CompletionsViewData] (backed by
/// `completion_events`).
///
/// [completedAt] aliases [CompletionsViewData.eventTimestamp].
/// [derivedFromEvents] is always `true` for view-sourced rows.
class Completion {
  const Completion({
    required this.id,
...
/// [CompletionEvents] filtered by `purgedAt IS NULL`. The former
/// `completions` table was dropped in W3.20; all callers continue to
/// receive [Completion] objects via [_fromView].
///
...
class CompletionDao extends DatabaseAccessor<UserDatabase>
  ...
  /// Maps a [CompletionsViewData] row to a [Completion] object.
  ///
  /// `completedAt` is taken from `eventTimestamp` (same instant, different
  /// column name).  `derivedFromEvents` is always `true` for view rows.
