...
///
/// Wraps [ZmanimWindowService] with an in-memory cache:
///
/// A `List<SacredWindow>?` keyed on (lat, lng, inIsrael), identical to the
/// original implementation, so synchronous callers ([NotificationScheduler])
/// are unaffected.
///
/// **No DB tier.** The Drift-era repository also wrote computed windows to a
/// device-local user DB (fire-and-forget) so background notification-fire-time
/// checks could read them on cold-start without the Flutter engine (AC 26.24
/// requirement 4). That tier's storage engine (the Drift user DB) is deleted,
/// and `docs/firestore-rewrite-map.md` keeps the derived zmanim cache
/// device-local ("Stays local, never leaves the device") — it is not a
/// Firestore migration target, so the persistence tier is dropped outright
