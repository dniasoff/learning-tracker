...
/// Seeds a second profile into the account's Firestore profile collection.
Future<void> _seedSecondProfile(
  E2EHarness h, {
  required String accountId,
  String displayName = 'Profile B',
  String profileId = '01J6Q2H4A8M7K3P9R5T6V8WXX',
}) async {
  await seedProfile(
    h.firestore,
    uid: accountId,
...
Future<List<Map<String, dynamic>>> _profileData(
  E2EHarness h,
  String uid,
) async {
  ...
  group('E2E-701 — Create first child profile with PIN', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        // ── Firestore assertion ──────────────────────────────────────────────
        final profiles = await _profileData(h, identity.accountId);
        final benny = profiles
            .where((p) => p['display_name'] == 'Benny')
            .firstOrNull;
        expect(
          benny,
          isNotNull,
          reason: 'Benny child profile must be in Firestore',
        );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final sarah = profiles
            .where((p) => p['display_name'] == 'Sarah')
            .firstOrNull;
        expect(
          sarah,
          isNotNull,
          reason: 'Sarah adult profile must be in Firestore',
        );
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          final profileB = profiles
              .where((p) => p['display_name'] == 'Profile B')
              .firstOrNull;
  ...
  group('E2E-706 — First-time PIN setup via route guard — no PIN set yet', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        // ── Firestore assertion ────────────────────────────────────────────
        // No profile document change expected — PIN is stored in SecureStorage.
        final profiles = await _profileData(h, identity.accountId);
  ...
  group('E2E-720 — Auto-select single profile on cold start', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final profiles = await _profileData(h, identity.accountId);
        expect(profiles, hasLength(1));
        expect(profiles.first['display_name'], 'Solo User');
      },
    );
