...
void main() {
  ...
  group('E2E-707 — Exit parent mode via ChildViewBanner', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(profile.data()!['mode'], 'child');
      },
    );
  });
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final profiles = await h.firestore
            ...
            .collection('learner_profiles')
            .get();
        final profileC = profiles.docs
            .where((p) => p.data()['display_name'] == 'Profile C')
            .firstOrNull;
        expect(profileC, isNotNull, reason: 'Profile C must be in Firestore');
        expect(profileC!.data()['mode'], 'adult');
      },
    );
  });
  ...
  group('E2E-709 — Rename profile via manage sheet (ManageLearnersScreen)', () {
    ...
    testWidgets('ManageLearnersScreen shows profiles; tap Edit on profile; '
      ...
      // ── DB assertion ──────────────────────────────────────────────────────
      final updated = await h.firestore
          .collection('users')
          .doc(identity.accountId)
          .collection('learner_profiles')
          .doc(identity.profileId)
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await seedProfile(
          ...
          profileId: '01J6Q2H4A8M7K3P9R5T6V8WXYZ',
        ...
        // ── DB assertion ────────────────────────────────────────────────────
        final remaining = await h.firestore
            .collection('users')
            .doc(identity.accountId)
            .collection('learner_profiles')
            .get();
        expect(
          ...
          1,
          reason: 'Should have 1 profile remaining',
        );
        expect(
          remaining.docs.any((p) => p.data()['display_name'] == 'Profile B'),
          isTrue,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        const tutoredSelection = TutoredProfileSelection(
          ...
          tutorOwnProfileId: 'tutor-own-716',
        );
