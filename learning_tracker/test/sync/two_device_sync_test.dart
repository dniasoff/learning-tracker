...
void main() {
  group('I-5: two-device sync scenarios', () {
    ...
    test('S1: completion on Device A propagates to Device B after sync', () async {
      ...
      expect(
        ...
        isTrue,
        reason: 'S1: completion event must appear on Device B after push+pull',
      );
    });
    ...
    test('S2: deleteTrackAndData enqueues a track_delete outbox row', () async {
      ...
      expect(
        ...
        isTrue,
        reason: 'I-5: soft-delete must enqueue track_delete so remote devices sync',
      );
    });

    // ── S3: same completion from two devices deduplicates ────────────────

    test('S3: same completion from two devices collapses to one event', () async {
      ...
      final matching = (await deviceB.completionEventDao.getEventsByProfile(1))
          .where((e) => e.sefariaRef == 'Berakhot 5:1')
          .toList();

      expect(
        matching,
        hasLength(1),
        reason: 'S3: UNIQUE constraint must collapse duplicate events to one row',
      );
    });
  });
}
