...
void main() {
  group('v15→v16: derived_from_events column', () {
    ...
    test('completions table has derived_from_events with default false', () async {
      final db = inMemoryDb();
    ...
    test('CompletionWriter sets derived_from_events = true on new writes', () async {
      final db = inMemoryDb();
      addTearDown(db.close);
      await seedProfile(db);
    ...
    test('duplicate commit returns isNew=false without writing a second event', () async {
      final db = inMemoryDb();
      addTearDown(db.close);
      await seedProfile(db);
    ...
    test('migration backfill: existing completion + event gets derived=true; orphan stays false', () async {
      ...
      final berakhot2a = rows.firstWhere((r) => r.sefariaRef == 'Berakhot 2a');
      final berakhot2b = rows.firstWhere((r) => r.sefariaRef == 'Berakhot 2b');

      expect(
        berakhot2a.derivedFromEvents,
        isTrue,
        reason: 'backfill must set derived_from_events=true when a matching '
            'completion_events row exists',
      );
      expect(
        berakhot2b.derivedFromEvents,
        isFalse,
        reason: 'completion without a matching event must keep derived_from_events=false',
      );
    });
  });
}
