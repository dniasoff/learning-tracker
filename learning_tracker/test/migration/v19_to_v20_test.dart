...
void main() {
  group('v19→v20: completion_events columns + completions_view purge filter', () {
    ...
    test(
      'track_id column: insert with non-null trackId and read it back',
      () async {
        final db = inMemoryDb();
        ...
        expect(
          ...
          isNotNull,
          reason:
              'C1/v20: track_id column must exist and accept a non-null value',
        );
    ...
    test('completions_view excludes rows where purgedAt is set', () async {
      ...
      await (db.update(
        db.completionEvents,
      )..where((t) => t.sefariaRef.equals('Berakhot 4a'))).write(
        CompletionEventsCompanion(purgedAt: Value(DateTime.utc(2026, 1, 5))),
      );
    ...
    test(
      'completions_view returns only the 1 non-purged row out of 2 inserted',
      () async {
        final db = inMemoryDb();
        addTearDown(db.close);
        ...
        await (db.update(
          db.completionEvents,
        )..where((t) => t.sefariaRef.equals('Berakhot 6a'))).write(
          CompletionEventsCompanion(purgedAt: Value(DateTime.utc(2026, 1, 7))),
        );
        ...
        expect(
          allEvents,
          hasLength(2),
          reason: 'sanity: both rows in completion_events',
        );
