...
void main() {
  group(
    ...
    () {
      ...
      test('restoreOrCreate is idempotent on an already-active track', () async {
        const profileId = 1;
        const curriculum = CurriculumId.mishnayos;
        const trackType = TrackType.personal;
      ...
      test(
        ...
        () async {
          ...
          final before = await (db.select(db.completions)
          ...
          final after = await (db.select(db.completions)
                ..where((t) => t.trackId.equals(trackId)))
              .get();
