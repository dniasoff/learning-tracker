...
/// the `completions` projection row AND the `outbox` row atomically.
@Tags(['epic_25'])
library;
...
void main() {
  group(
    ...
    () {
      ...
      test(
        'commit() inserts a completion row AND an outbox row in one transaction',
        () async {
      ...
      test(
        ...
        'for a duplicate (profileId, sefariaRef, stageId, trackType)',
        () async {
