...
void main() {
  group('Story 15 — profile identity and isolation', tags: ['story_15'], () {
    test('two Firestore profiles have independent document paths', () async {
      ...
      final first = await repository.getProfile('01J00000000000000000000004');
      final second = await repository.getProfile('01J00000000000000000000005');
    ...
    test(
      'curriculum tracks are keyed by CurriculumId, never an int id',
      () async {
        final firestore = FakeFirebaseFirestore();
        await seedTrack(
          firestore,
  ...
  group(
    'Story 15 — legacy Drift profile suites',
    skip:
    ...
    () {
      test(
        'placeholder for the pending Firestore multi-profile service seam',
        () {},
      );
    },
  );
