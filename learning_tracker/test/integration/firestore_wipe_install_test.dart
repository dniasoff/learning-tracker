...
Future<void> _appendEvent(UserDatabase db, Map<String, dynamic> row) =>
...
void main() {
  group('DNI-343 — Firestore wipe-install round-trip', () {
    // ── AC1: full round-trip ─────────────────────────────────────────────

    test('completions pushed on device A are fully restored on device B '
        'after a local DB wipe', () async {
    ...
    test('second pull is idempotent — duplicate fetchPage does not insert '
        'duplicate completion_events rows (INSERT OR IGNORE)', () async {
      final db = inMemoryDb();
    ...
    test('two in-memory DB instances are independent — device B starts clean '
        'regardless of data in device A', () async {
      // Device A writes a track.
      final dbA = inMemoryDb();
      await dbA
          .into(dbA.curriculumTracks)
          .insert(
