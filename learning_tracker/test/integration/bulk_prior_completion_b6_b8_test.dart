  ...
  group('B6 — execute writes all configured stages for prior-marked items', () {
    test(
      ...
      () {
        ...
        _retired(
          'The assertion is migrated to the repository-seam B6 tests; local completion_events rows no longer exist.',
        );
      },
    );
    test(
      ...
      () {
        ...
        _retired(
          'The old row-level timestamp assertion has no local table to inspect; current sentinel behavior is owned by BulkPriorCompletionService.execute.',
        );
      },
    );
  });
  ...
  group('B8 — expungePriorCompletions tombstones only sentinel-dated rows', () {
    test(
      ...
      () {
        ...
        _retired(
          'The equivalent current purge assertion is in bulk_prior_completion_service_test.dart; prior_completion_imports was deleted.',
        );
      },
    );
    test('AC2: does NOT touch rows for a different sefariaRef', () {
      ...
      _retired(
        'The old completion_events/prior_completion_imports fixture has no current table equivalent.',
      );
    });
    test(
      'AC3: does NOT touch live-learning rows (non-sentinel timestamp) for the same sefariaRef',
      () {
        ...
        _retired(
          'The timestamp-based local assertion is replaced by current source/provenance coverage.',
        );
      },
    );
    test(
      'AC4: expunge is idempotent — calling twice does not throw and purgedAt remains set',
      () {
        // Per-case disclosure: idempotency now belongs to Firestore tombstones and the D-M ledger epoch rule.
        _retired(
          'The current idempotency coverage is in bulk_prior_completion_service_test.dart; the old Drift purgedAt row is retired.',
        );
      },
    );
    test(
      'AC5: expunge on a sefariaRef with no prior rows is a no-op (does not throw)',
      () {
        // Per-case disclosure: no-prior-row is no longer a query against `prior_completion_imports`.
        _retired(
          'The old local no-op premise is retired; current expunge requires its Firestore retraction collaborators.',
        );
      },
    );
    test('AC6: does NOT touch rows belonging to a different profileId', () {
      ...
      _retired(
        'The integer profile-FK fixture cannot be migrated without inventing a second current session.',
      );
    });
  });
  ...
  group('Finding 8 — per-curriculum expunge isolation', () {
    test(
      ...
      () {
        ...
        _retired(
          'Current curriculum isolation is expressed by CompletionRepository.purgeCompletion(curriculumId: ...), covered by the current service tests.',
        );
      },
    );
    test(
      ...
      () {
        ...
        _retired(
          'The current Firestore document identity and curriculum field replace the old two-track local fixture.',
        );
      },
    );
    test(
      ...
      () {
        ...
        _retired(
          'Current source filtering is covered in bulk_prior_completion_service_test.dart; no local row remains to inspect.',
        );
      },
    );
    test(
      ...
      () {
        ...
        _retired(
          'The old upgrade premise has no current Firestore-native equivalent in this integration fixture.',
        );
      },
    );
  });
