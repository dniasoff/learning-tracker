...
/// regression the next time someone touched this file. This is that probe,
/// made permanent, extended to cover `ensureDefaultProfile` too, plus a
/// third case (`ensureRemoteProfile`) so all three callers of
/// `_ensureFirestoreProfile` are proven, in one place, never to reintroduce
/// this clobber.
///
...
/// delayed/awaitable Firestore double so one profile's write is still in
/// flight while a DIFFERENT, newer profile becomes the one actually
/// selected — the identical out-of-order-completion shape the real bug has,
/// driven by an injected delay rather than a real network partition.
library;
