...
void main() {
  ...
  testWidgets(
    'B1: "priest" keyword in English text → "Vocabulary: Priests" chip',
    (tester) async {
      await tester.pumpWidget(
        _buildApp(
          router: router,
          content: _content(
            hebrewText: '',
            englishText: 'The priest went to the Temple.',
          ),
        ),
      );
      await tester.pump();
      ...
      expect(find.text('Vocabulary: Priests'), findsOneWidget);

      await _tearDown(tester);
    },
  );

  testWidgets('B2: "time" keyword in English text → "Concept: Time" chip', (
    tester,
  ) async {
    await tester.pumpWidget(
      _buildApp(
        ...
        content: _content(
          hebrewText: '',
          englishText: 'At what time should one recite the Shema?',
        ),
      ),
    );
    await tester.pump();
    ...
    expect(find.text('Concept: Time'), findsOneWidget);

    await _tearDown(tester);
  });

  testWidgets('B3: "watch" keyword in English text → "Concept: Time" chip', (
    tester,
  ) async {
    await tester.pumpWidget(
      _buildApp(
        ...
        content: _content(
          hebrewText: '',
          englishText: 'One must watch the stars carefully.',
        ),
      ),
    );
    await tester.pump();
  ...
  testWidgets('B4: No special keywords → no insight chips rendered', (
    tester,
  ) async {
    await tester.pumpWidget(
      _buildApp(
    ...
    expect(find.text('Vocabulary: Priests'), findsNothing);
    expect(find.text('Concept: Time'), findsNothing);

    await _tearDown(tester);
  });

  testWidgets(
    'B5: "priest" + "time" keywords → both chips shown (capped at 2)',
    (tester) async {
      await tester.pumpWidget(
        _buildApp(
          router: router,
          content: _content(
            hebrewText: '',
            englishText:
                'The priest must observe the correct time for the sacrifice.',
          ),
        ),
      );
      await tester.pump();
