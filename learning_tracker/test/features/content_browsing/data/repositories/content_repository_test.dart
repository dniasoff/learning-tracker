...
void main() {
  ...
  group(
    'ContentRepository Acceptance Tests',
    ...
    () {
      ...
      test(
        'AT2: Filtering by Seder Zeraim returns children sorted by sortOrder',
        () async {
          ...
          final sederZeraim = allItems.firstWhere(
            (item) =>
                item.displayNameEn.contains('Zeraim') || item.level1 != '',
      ...
      test(
        'AT4: getHierarchyConfig returns correct level labels for Mishnayos',
        () async {
      ...
      test(
        'filterByLevel with multiple level constraints returns correct items',
        () async {
          final allItems = await repository.getContentForCurriculum(
            CurriculumId.mishnayos,
          );
      ...
      test('search returns items matching Hebrew or English query', () async {
        ...
        final hasMatch = results.any(
          (item) =>
              item.displayNameEn.toLowerCase().contains('berakhot') ||
