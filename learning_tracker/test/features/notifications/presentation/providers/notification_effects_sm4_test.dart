...
///
/// BEFORE: reminderSyncEffect watched `activeProfileIdProvider` up front,
/// then later awaited `ref.watch(reminderEnabledProvider.future)` with zero
/// `ref.mounted` guard before the next `ref` touch. If `activeProfileIdProvider`
...
/// activeProfileIdProvider override mid-flight, THEN resolve the gate —
...
void main() {
  ...
  test('reminderSyncEffect: activeProfileIdProvider changing while '
    ...
    List<Override> overridesFor(int profileId) => [
    ...
    final container = ProviderContainer(overrides: overridesFor(1));
    ...
    // Kick off the build — it watches activeProfileIdProvider (id=1) up
    ...
    // Profile switch mid-flight: the watched activeProfileIdProvider
    ...
    // must defend against.
    container.updateOverrides(overridesFor(2));
