...
void main() {
  ...
  // =========================================================================
  // CurriculumProgressService.computeCompletionPercentage (static)
  // =========================================================================

  group('CurriculumProgressService.computeCompletionPercentage', () {
    ContentItem leaf(String ref, int sortOrder) => ContentItem(
      curriculumId: 'mishnayos',
      level1: 'Seder Zeraim',
      level2: 'Berakhot',
      displayNameHe: ref,
    ...
    test('returns 0.0 when leafItems is empty', () {
      final result = CurriculumProgressService.computeCompletionPercentage(
        leafItems: [],
        completions: [],
      );
      expect(result, 0.0);
    ...
    test('returns 0.0 when no completions', () {
      final items = [leaf('Ref A', 0), leaf('Ref B', 1)];
      final result = CurriculumProgressService.computeCompletionPercentage(
        leafItems: items,
        completions: [],
      );
    ...
    test('returns 1.0 when all items are completed', () async {
      final items = [leaf('Ref A', 0), leaf('Ref B', 1)];

      // Insert completions so we have real Completion rows.
      ...
      final allCompletions = await db.completionDao.getCompletionsByProfile(1);
      final result = CurriculumProgressService.computeCompletionPercentage(
        leafItems: items,
    ...
    test('returns 0.5 when half the items are completed', () async {
      final items = [leaf('Ref C', 0), leaf('Ref D', 1)];
