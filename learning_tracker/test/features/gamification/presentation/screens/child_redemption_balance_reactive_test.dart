...
void main() {
  group(
    'childRedemptionBalanceProvider — reactive stream (DG-RDMP-01)',
    () {
      test(
        'childRedemptionBalanceProvider re-emits updated balance after '
        'parentAdjust WITHOUT explicit invalidation (must be stream-backed, '
        'not one-shot FutureProvider)',
        () async {
          final db = inMemoryDb();
      ...
      test(
        'childRedemptionBalanceProvider re-emits debited balance after '
        'createRedemption WITHOUT explicit invalidation',
        () async {
          final db = inMemoryDb();
          addTearDown(db.close);
          await seedProfile(db);
