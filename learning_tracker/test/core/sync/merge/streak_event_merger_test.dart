...
void main() {
  group('StreakEventMerger', () {
    test('C2 regression: W3.37 shape (study_date) is inserted into Drift',
        () async {
    ...
    test('C2 regression: legacy event_timestamp shape is still accepted',
        () async {
    ...
    test('C2 regression: row missing both study_date and event_timestamp is skipped',
        () async {
      final db = createTestDatabase();
      await seedProfile(db);
      final profiles = await db.select(db.learnerProfiles).get();
