...
void main() {
  ...
  group('2a. FB-2 — server timestamps for LWW ordering fields '
    ...
    test('pushRewardRedemption overwrites updated_at (the field '
        'RewardRedemptionMerger compares) with a server Timestamp', () async {
      final fs = createFakeFirestore(authenticatedUid: _uid);
      await _gw(fs).pushRewardRedemption(
        profileId: _profileId,
        data: {
    ...
    test('pushNotificationSettings overwrites updated_at (the field '
        'NotificationSettingsMerger compares) with a server Timestamp '
        '(AUD-notifications-07)', () async {
      final fs = createFakeFirestore(authenticatedUid: _uid);
      await _gw(fs).pushNotificationSettings(
        profileId: _profileId,
        data: {
          'daily_reminder': {'enabled': true, 'hour': 19, 'minute': 0},
          'updated_at': clientClockValue,
        },
      );
      final snap = await fs
