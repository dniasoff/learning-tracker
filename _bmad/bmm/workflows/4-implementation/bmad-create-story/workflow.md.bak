...
<step n="1" goal="Determine target story">
  ...
  <check if="{tracking_system} == linear">
    <critical>Linear is the SOLE source of truth. Read from .linear-cache/ (not sprint-status.yaml in implementation-artifacts).</critical>
    ...
    <action>Read `.linear-cache/sprint-status.yaml` and find stories with status Backlog</action>
    ...
    <check if="no backlog stories found in Linear">
      <output>📋 No backlog stories found in Linear for team {team_key}
    ...
    <!-- Check epic status in Linear if this is first story -->
    <action>Check if this is the first story in epic {{epic_num}}</action>
    <check if="this is first story in epic {{epic_num}}">
      <action>Query Linear for epic status:
        linearis issues search "[Epic {{epic_num}}]" --team {team_key} --limit 1 \
          | jq '.[0] | {id: .identifier, status: .state.name}'
      </action>
