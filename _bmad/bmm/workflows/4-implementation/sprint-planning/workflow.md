...
<check if="{tracking_system} == linear">
  <action>Create Linear issues for each epic and story:
    For each epic:
      linearis issues create "[Epic-N] Epic Title" --team {team_key} --project {project_name} \
        --labels "BMAD-Managed,Epic" -d "Epic goal statement"
    For each story:
      linearis issues create "[N-M-key] Story Title" --team {team_key} --project {project_name} \
        --labels "BMAD-Managed,Story" --parent-ticket [epic-id] \
        -d "Story placeholder — run create-story to add Dev Notes" --status "Backlog"
  </action>
  <action>Create linear-mapping.yaml with team_key, project_name, and issue ID mappings</action>
  <action>Run `tool/linear-sync.sh sync` to populate .linear-cache/ with all issue data</action>
</check>
</step>
...
<check if="{tracking_system} == linear">
  <action>Use `.linear-cache/sprint-status.yaml` as the validation source (NOT {status_file})</action>
  <action>If `.linear-cache/sprint-status.yaml` is missing, instruct user to run `make linear-sync`</action>
</check>

<action>Perform validation checks (use {status_file} for file-system mode, `.linear-cache/sprint-status.yaml` for linear mode):</action>
...
- **Local Cache:** `.linear-cache/sprint-status.yaml`
- **Total Epics:** {{epic_count}}
- **Total Stories:** {{story_count}}
- **Epics In Progress:** {{in_progress_count}}
- **Stories Completed:** {{done_count}}
