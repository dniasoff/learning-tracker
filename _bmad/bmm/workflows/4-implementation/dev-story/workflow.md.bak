...
linear_mapping: "{implementation_artifacts}/linear-mapping.yaml"
---
  ...
  <step n="1" goal="Find next ready story and load it" tag="sprint-status">
    ...
    <check if="{tracking_system} == linear">
      ...
      <check if="{{story_key}} is provided">
        <action>Read story details from `.linear-cache/stories/{{story_key}}.yaml`</action>
      ...
      <action>Read `.linear-cache/sprint-status.yaml` and find stories with status Todo</action>
...
Run `make linear-sync` to refresh, or run create-story / sprint-planning.</output>
      ...
      <action>Read full story details from `.linear-cache/stories/{{linear_issue_id}}.yaml`</action>
      ...
      <action>Get comments for review continuation check:
        linearis issues read {{linear_issue_id}} | jq '[.comments[] | {author: .user.name, body: .body}]'
      </action>
    </check>
  ...
  <step n="8" goal="Validate and mark task complete ONLY when fully done">
    ...
    <check if="{tracking_system} == linear">
      <action>Tick checkbox in Linear issue description:
        DESC=$(linearis issues read {{linear_issue_id}} | jq -r '.description' | sed 's/- \[ \] TASK_TEXT/- [x] TASK_TEXT/')
        linearis issues update {{linear_issue_id}} -d "$DESC"
      </action>
  ...
  <step n="9" goal="Story completion and mark for review" tag="sprint-status">
    ...
    <!-- Linear completion -->
    <check if="{tracking_system} == linear">
      <action>Verify all Linear checkboxes ticked:
        linearis issues read {{linear_issue_id}} | jq -r '.description' | grep -c '- \[ \]'
      </action>
