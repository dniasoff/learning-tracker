...
<check if="{tracking_system} == linear">
  ...
  <action>Read `.linear-cache/sprint-status.yaml` and filter for stories with status Done</action>
  <action>Find the highest epic number with completed stories from cached data</action>
  <action>For detailed story comments/learnings, read `.linear-cache/stories/DNI-XX.yaml` for each completed story</action>
</check>
