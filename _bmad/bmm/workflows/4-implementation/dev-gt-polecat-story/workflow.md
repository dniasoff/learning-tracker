...
description: 'Gastown polecat story. Inherits from dev-gt-story. Adds BD_BRANCH verification and gt mol step done tracking.'
---
...
```bash
echo "POLECAT=$GT_POLECAT BRANCH=$BD_BRANCH ROLE=$GT_ROLE AGENT=$BEADS_AGENT_NAME"
```
...
<check if="BD_BRANCH is not set">
  HALT: BD_BRANCH is not set. Polecat sandbox was not properly initialized.
</check>

### Step 1: Context (APPEND — add gt mol + polecat identity)

Include in Linear comment: **Polecat:** $GT_POLECAT, **Branch:** $BD_BRANCH

```bash
gt mol step done load-context --body "Context Loaded. Story: {{issue_identifier}}, Polecat: $GT_POLECAT, Branch: $BD_BRANCH"
```

### Step 2: Verify polecat branch (REPLACES base Step 2)

Polecats use a pre-created branch ($BD_BRANCH). Do NOT create a new branch.

```bash
git branch --show-current  # Expected: $BD_BRANCH
git fetch origin
...
```bash
gt mol step done branch-setup --body "Branch: $BD_BRANCH, rebased on {default_branch}"
```
