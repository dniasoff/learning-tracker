...
> **READS:** Use `.linear-cache/sprint-status.yaml` for status data and `.linear-cache/stories/DNI-XX.yaml`
...
<step n="2" goal="Read and parse sprint data">
  ...
  <check if="{tracking_system} == linear">
    <critical>Linear is the SOLE source of truth. Read status from .linear-cache/ (not sprint-status.yaml in implementation-artifacts).</critical>
    <check if=".linear-cache/sprint-status.yaml does not exist">
      ...
      <check if="sync command failed">
        <output>❌ Failed to sync Linear cache. Check that `linearis` CLI is configured and the team key is correct.</output>
        <action>Exit workflow</action>
      </check>
      <output>✅ Linear cache created successfully.</output>
    </check>
    <action>Read `.linear-cache/sprint-status.yaml` for all epics and stories with statuses</action>
    ...
    <action>Parse project metadata from the cache file (project, team_key)</action>
  </check>
...
- Status file: {{#if tracking_system == "linear"}}.linear-cache/sprint-status.yaml{{else}}{sprint_status_file}{{/if}}
...
<step n="30" goal="Validate sprint tracking">
  <check if="{tracking_system} == linear">
    <action>Query Linear to verify team issues are accessible:
      linearis issues search "" --team {team_key} --limit 1 | jq 'length'
    </action>
    <check if="query fails or returns 0">
      <template-output>is_valid = false</template-output>
      <template-output>error = "Linear query failed or returned no issues for team {team_key}"</template-output>
      <template-output>suggestion = "Check linearis auth and team_key in config.yaml"</template-output>
      <action>Return</action>
    </check>
    <template-output>is_valid = true</template-output>
    <template-output>message = "Linear tracking valid: team {team_key} accessible with issues"</template-output>
    <action>Return</action>
  </check>
