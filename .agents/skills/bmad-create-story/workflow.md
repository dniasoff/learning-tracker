...
> **Linear Integration:** When {tracking_system} == linear, use `linearis` CLI instead of local files. Always pipe through jq.
...
<step n="1" goal="Determine target story">
  <check if="{tracking_system} == linear">
    <action>Query Linear backlog for next story:
      linearis issues search "" --team {team_key} --status Backlog --limit 10 \
        | jq '[.[] | {id: .identifier, title: .title, status: .state.name}]'
    </action>
    <action>Use Linear backlog results to identify target story if no user input provided</action>
  </check>
...
<step n="5" goal="Create comprehensive story file">
  ...
  <check if="{tracking_system} == linear">
    <action>Write Dev Notes to Linear issue description:
