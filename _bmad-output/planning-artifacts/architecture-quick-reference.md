...
**Learning Tracker** is a multi-curriculum Torah learning app (Flutter/Dart, Android-only) supporting:
- **5 Curricula:** Mishnayos, Gemara Bavli, Yerushalmi, Mishna Berurah, Chumash (via Sefaria API)
- **User Modes:** Child (gamified, parent-managed) & Adult (self-directed, optional gamification)
- **Core Features:** Multi-stage learning (Learn → Chazara 1 → Chazara 2...), smart scheduling, multi-track (personal/school/tutor), offline-first with cloud sync

**Tech Stack:** drift (SQLite ORM), auto_route 11.x, Riverpod, freezed, dio, talker, Firebase Auth + Firestore, kosher_dart, flutter_secure_storage, bcrypt
