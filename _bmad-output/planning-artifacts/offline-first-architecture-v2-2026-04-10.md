...
> ⚠️ **Deliberate tech debt:** Production code in `learning_tracker/lib/` currently implements the deleted March architecture — sealed `AppAuthState`, `LocalAuthGuard`, anonymous `localUid`, deferred/optional account creation. The code refactor to bring the implementation in line with this v2 doc is tracked as a separate Linear epic (see §7 follow-up #8). Until that epic lands, the docs describe the target architecture and the code describes the current architecture, and the two will not match. This gap is intentional and scheduled — not accidental debt.
...
- `_bmad-output/planning-artifacts/local-first-auth-abstraction-layer.md` (~822 lines) — was entirely superseded; removed from the repo.
- `_bmad-output/planning-artifacts/offline-first-analysis-2026-03-27.md` (~677 lines) — content still valid was absorbed into §5 (Inherited Unchanged) of this doc and into the still-canon companion docs (`two-database-drift-architecture.md`, `calendar-cycle-computation-analysis.md`).
