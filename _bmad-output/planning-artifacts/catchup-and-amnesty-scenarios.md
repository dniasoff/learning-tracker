...
created: 2026-04-10
updated: 2026-04-10
status: draft-v2 (all 20 open questions + 4 NQs resolved)
...
All four resolved 2026-04-13.

- **NQ1** — *Children's overrides of `primary_unit_type`*: is this set during Setup Seeding or only from Track settings? **Resolved: both.** Available as an optional choice during initial track creation (defaults to parent track's unit type), and editable anytime in Track Settings as the long-term override home.
- **NQ2** — *Retrofit review seeding*: how are declared-at-setup reviews recorded? **Resolved: real completion records** with `completedAt = now` and a `source = "retrofit_seeding"` tag. No backdating — the learner is declaring progress now, and the timestamp reflects that. Honest audit trail; no special-casing downstream in PaceCalculator or debt queries.
- **NQ3** — *Welcome-back after pause vs. after dormancy*: are these the same UI treatment or distinct? **Resolved: distinct.** Pause is a coping mechanism for being overwhelmed by debt (not a planned holiday), so pause-return tone is supportive and action-oriented ("Ready to get back on track? Let's see where things stand.") and routes straight into triage. Dormancy-return tone is warm re-engagement with no judgement ("Great to see you again!") and also routes to triage. Same skeleton, different emotional framing; WDS designs both variants.
- **NQ4** — *Pause during triage*: if the learner pauses a track during triage, does triage skip that track? **Resolved: yes, immediately.** Pause means "make this track's debt invisible" — a paused track drops out of the triage sequence the moment it is paused. If it was the last remaining track, triage ends gracefully.
