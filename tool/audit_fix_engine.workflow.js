...
const DOCTRINE = [
  ...
  '- EVERY acceptance_criterion is mandatory DoD, NOT optional. Never defer an AC as a "follow-up"/"out of scope"/"disproportionate"/"not required"/"none exists yet" and merge anyway - that is FAKE-DONE and will be caught. AC wording like "a lint rule (none exists yet)" or "add a checker" is an INSTRUCTION TO CREATE the artifact, never a statement that it is optional. A working template exists: packages/custom_lints/lib/src/rules/no_hand_rolled_async_state_notifier.dart (rule) + its test asserting the lint FIRES on a violation + registration in packages/custom_lints/lib/learning_tracker_lints.dart - copy that shape for your finding pattern. If an AC names a Rule-0 checker (a custom_lint rule under packages/custom_lints, or a make audit grep that FAILS on the violation), you MUST ship it and demonstrate it fires on a deliberately-broken fixture then passes clean (custom_lint is repaired as of Wave 0 - it works). The distinction: an ADJACENT DEFECT (a similar bug elsewhere the finding does not name) is a follow-up; an ACCEPTANCE CRITERION of THIS finding is never optional. If you genuinely cannot satisfy an AC, return the finding as outcome=blocked with the specific reason - do NOT report it fixed.',
...
const GATES = [
  ...
  'Timeout discipline (HARD RULE): a hung test once stalled the engine for 5 hours. Wrap EVERY test/codegen invocation in a hard timeout - "timeout 600 flutter test ...", "timeout 900 dart run build_runner ...", "timeout 300 dart run custom_lint". If a command times out (exit 124), treat it as a failing gate: do NOT commit, investigate the hang, and if you cannot resolve it, return outcome=blocked with the exact command - never leave a process hanging.',
...
function reviewerPrompt(cid, tipSha, dispatchIds, buildJson) {
  ...
  lines.push('2. Verify EVERY acceptance criterion of every claimed-fixed finding INDIVIDUALLY, against the actual tree, with evidence (file:line or command output). An AC you cannot verify = bounce. A builder DEFERRING an AC as a "follow-up"/"out of scope"/"disproportionate"/"candidate" is FAKE-DONE, not a scope decision - the acceptance_criteria ARE the scope: verdict=bounce. This applies especially to Rule-0 checker ACs (a custom_lint rule or a make audit grep that must fail on a violation): confirm the checker artifact EXISTS and actually fires on a violation - "hand-fixed the sites but skipped the checker" = bounce.')
