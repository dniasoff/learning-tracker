...
const BATCH = 'R4'
const SCREENS = [
  { slug: 'r4_parent_settings', serial: 'emulator-5554', nav: 'Parent Settings hub (Settings, as an ADULT profile — parent surfaces need no PIN when already adult). Audit every tile/row, section headers, account/header card, and any toggles. Check for clipping, stale state, half-translation, and that no child-mode badge appears on this parent surface. font 1.0/1.3, en. (Set the DEVICE language to Hebrew via system Settings only if quick; otherwise note skipped.)', controls: 'every settings tile, account header, section toggles, navigation rows' },
  { slug: 'r4_manage_learners', serial: 'emulator-5556', nav: 'Manage Learners / Manage Profiles (Settings → Manage Profiles). On 5556 the parent PIN is 2580 if gated. Audit the learner list, add-learner CTA, per-learner edit/delete, avatars, mode badges. Verify no overflow at 1.3, correct pluralization on any counts, and that delete has a safe confirm. font 1.0/1.3.', controls: 'learner rows, add-learner FAB, edit/delete, avatars, mode badges, confirm dialog' },
  { slug: 'r4_manage_tutors', serial: 'emulator-5558', nav: 'Manage Tutors (Settings → Parental/Tutoring → Manage Tutors). Audit the tutor list, invite CTA, pending vs accepted states, revoke. Verify states render correctly (no stale "Pending"), no clipping, correct empty state. font 1.0/1.3.', controls: 'tutor rows, invite CTA, pending/accepted badges, revoke, empty state' },
  { slug: 'r4_invite_tutor', serial: 'emulator-5554', nav: 'Invite Tutor (Manage Tutors → Invite, or Settings → Invite Tutor). Audit the email input (junk/empty/huge input probes), send button enabled/disabled states, any generated invite link/code, validation errors. Verify a11y labels on the email field and no raw error strings. font 1.3.', controls: 'email field, send button, invite code/link, validation error, copy/share' },
  { slug: 'r4_notifications', serial: 'emulator-5556', nav: 'Notification Settings (Settings → Notification Settings). Audit every toggle (daily reminder, etc.) — set one OFF, relaunch, confirm it PERSISTS off (the iter-11 bug class). Time pickers, permission state. Verify the notification icon is not a saturated-red disc inconsistent with the pastel palette (R2 note). font 1.3.', controls: 'every toggle, time picker, permission row, reminder settings' },
  { slug: 'r4_city_picker', serial: 'emulator-5558', nav: 'City Picker (Settings → Shabbos Mode → Detect/Choose city → CityPickerScreen). Enter junk/empty/Hebrew queries; verify results render, no crash, and the subtitle does NOT show a raw GeoNames admin code "NN" (known-deferred if still present — note but do not re-flag). Verify selected city persists. font 1.3.', controls: 'search field, city result rows, detect-location, selected state, empty/no-results' },
]
