...
const BATCH = 'R5'
const SCREENS = [
  { slug: 'r5_upgrade_to_cloud', serial: 'emulator-5554', nav: 'Upgrade to Cloud (Settings → Backup & Sync card → "Upgrade to Cloud" button → UpgradeToCloudScreen). Audit the value-prop copy, sign-in/Google buttons, any benefit list/icons, error states. Verify no clipping/overflow at font 1.3, no raw error tokens, fully localized. (Test Hebrew via `cmd locale set-app-locales com.jcom.torah.learning_tracker --locales he` on this API-33+ device, then reset with --locales \'\'.)', controls: 'value-prop copy, sign-in/Google buttons, benefit list, back, error states' },
  { slug: 'r5_track_management_hub', serial: 'emulator-5556', nav: 'Track Management Hub (Settings → Manage Tracks → TrackManagementHubScreen). Audit the active/archived track list, add-track CTA, per-track actions (edit/archive/delete), status chips, and any counts (pluralization). Verify nusach-correct track/curriculum names (one spelling), no clipping at 1.3, safe delete confirm.', controls: 'track rows, add-track CTA, edit/archive/delete, status chips, empty state' },
  { slug: 'r5_curriculum_list', serial: 'emulator-5558', nav: 'Curriculum List (Add Track → Step 1 "Select what to learn" / curriculum picker). Audit the curriculum cards/rows, icons, descriptions, selection state. Verify nusach-correct names, no Hebrew-script leak in English mode, no clipping at 1.3, and the list scrolls cleanly.', controls: 'curriculum cards, icons, descriptions, select, search/filter, next' },
  { slug: 'r5_learning_order', serial: 'emulator-5554', nav: 'Learning Order (a track → its learning-order / sequence screen, TrackLearningOrderScreen — reachable from track detail or Manage Tracks). Audit the ordered list of units/masechtos, reorder affordance if any, current-position indicator, nusach-correct names (ONE spelling throughout — no Berakhos/Berakhot mix), no clipping at 1.3.', controls: 'ordered unit list, reorder handles, current marker, section headers' },
  { slug: 'r5_text_display', serial: 'emulator-5556', nav: 'Text Display / reader (Dashboard → a track → Start Learning → the daf/mishna/pasuk text reader, TextDisplayScreen). Audit the Hebrew text body (with/without nikud per pref), the AppBar breadcrumb chain (nusach-correct, not clipped), prev/next nav arrows, and the bottom mark-complete / next-daily-task buttons (NOT clipped by the nav-bar inset — recently SafeArea-fixed). font 1.3.', controls: 'content text body, nikud, AppBar breadcrumb, prev/next arrows, mark-complete/next buttons' },
  { slug: 'r5_lifetime_knowledge', serial: 'emulator-5558', nav: 'Lifetime Knowledge (Progress → Lifetime Knowledge / the per-curriculum learned-tree view). On 5558 use whatever profile has data. Audit the curriculum cards with learned %/counts, the drill-down tree (folder rows + node states full/partial/none), provenance labels (live/bulk/lifetime), nusach-correct names, and count pluralization. Verify the percentages are sane (the bare-daf collision is FIXED — marking should now credit only the right scope). font 1.3.', controls: 'curriculum cards, learned %/counts, drill-down tree, node-state indicators, provenance labels' },
]
