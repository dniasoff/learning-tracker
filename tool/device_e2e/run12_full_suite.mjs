export const meta = {
  name: 'device-e2e-full-suite-run12',
...
// (functions/logic unchanged) rather than imported.
...
function driveGuide(port) {
  ...
  return [
    ...
    `- RTL SegmentedButton auto-mirrors correctly via Directionality — a selected segment appearing on the LEFT in RTL is BY DESIGN, not a bug. Only flag a segmented control if it is INCONSISTENT with its sibling controls, or if the primary/first option lands on the TRAILING (left) edge instead of the leading (right) edge in RTL (independently relitigated in run-2/run-3/run-5 before this rule was centralized — do not rediscover it again).`,
...
function verifyPrompt(f) {
  return [
    `You are an adversarial verifier for an on-device audit finding. Decide if it is a REAL defect, a FALSE POSITIVE, BY_DESIGN, or NEEDS_DEVICE. Be skeptical — early runs had many false positives from auditors misreading intentional design, RTL conventions, and Flutter merged-semantics tap artifacts.`,
    ``,
    `## Finding`,
...
// ───────────────────────── run ─────────────────────────
const runNum = 12;
const reportIntro = `Run: comprehensive 3-device parallel on-device E2E re-audit (run 12 — first validation pass against the post-Firestore-migration build). Build = current dev HEAD f9818b55. This is the first on-device sweep since the Drift->Firestore migration finished (runs 1-11 all predate it, baseline dev HEAD f8b42240, 518 commits and a full rewrite of the sync/auth/data layer ago) -- treat this as a fresh baseline, not a continuation of the old reassurance-campaign scorecard. Full flutter test suite (783 files, all serialized-verified) and make audit are both green; this run checks what those can't see: real on-device UX/rendering/navigation behavior.`;
