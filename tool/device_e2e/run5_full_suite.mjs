...
return await runFullSuite({
  runNum: 5,
  reportIntro: `Run: comprehensive 3-device parallel on-device E2E re-audit (run 5 — fresh validation pass) of the Learning Tracker Android app. Build = current dev HEAD (all fixes from runs 1-4 + the 3 deferred fixes (cold-start, breadcrumb, dead-route) included; this run validates them).`,
