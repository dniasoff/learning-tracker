// Shared orchestration + prompt-builder module for the on-device E2E
// "full suite" runs (tool/device_e2e/runN_full_suite.mjs).
//
// AUD-guardrails-08: prior to this module, run2_full_suite.mjs through
// run7_full_suite.mjs were six 256-line files that differed from each other
// by EXACTLY 3 lines (meta.name, the OUT dir, and the report-intro string) —
// ~1536 lines of near-total copy-paste. Because the shared logic wasn't
// factored into one place, lessons learned in one run never propagated to
// the others: the RTL-segmented-controls false positive was independently
// rediscovered and excluded in run-3 (see run3_fix.mjs) and again in run-5
// (see run5_fix.mjs) before finally landing here, once, permanently.
//
// Each runN_full_suite.mjs now imports this module and supplies only its
// run-specific identity (a `meta` export for the harness, plus a runNum +
// reportIntro passed into runFullSuite()).

export const REPO = '/home/daniel/repos/learning-tracker';
export const DEVE2E = REPO + '/tool/device_e2e';
export const PKG = 'com.jcom.torah.learning_tracker';
...
export const SUITE_DESCRIPTION = 'Comprehensive 3-device parallel on-device E2E re-audit: seed each device, walk every screen across 16 feature areas (EN + Hebrew/RTL), adversarially verify every finding, synthesize a gate report';
...
export function driveGuide(port) {
  const d = DEV[port];
  return [
    ...
    ``,
    `## Judgment calibration (avoid known false positives — updated after each run, not just run 1)`,
    ...
    `- RTL SegmentedButton auto-mirrors correctly via Directionality — a selected segment appearing on the LEFT in RTL is BY DESIGN, not a bug. Only flag a segmented control if it is INCONSISTENT with its sibling controls, or if the primary/first option lands on the TRAILING (left) edge instead of the leading (right) edge in RTL (independently relitigated in run-2/run-3/run-5 before this rule was centralized here — do not rediscover it again).`,
...
// ───────────────────────── prompt builders ─────────────────────────
export function seedPrompt(port, spec) {
  return [
...
export function auditPrompt(port, area) {
  return [
...
export function verifyPrompt(f) {
  return [
...
// ───────────────────────── orchestration ─────────────────────────
// Runs the full seed -> audit -> verify -> synthesize pipeline for one
// numbered run. `phase`, `agent`, `parallel`, `log` are the harness-injected
// primitives from the caller's own scope (they are not module-level globals
// here, so each runN_full_suite.mjs must pass its own through explicitly).
export async function runFullSuite({ runNum, reportIntro, phase, agent, parallel, log, areas, seedSpecs }) {
  const OUT = `${REPO}/docs/test-artifacts/device-audit-run${runNum}`;
  const areaPartitions = areas || { '5560': AREAS_5560, '5554': AREAS_5554, '5562': AREAS_5562 };
  const specs = seedSpecs || SEED_SPECS;

  phase('Seed');
  const seedResults = (await parallel(['5560', '5554', '5562'].map(p =>
    () => agent(seedPrompt(p, specs[p]), { label: `seed:${p}`, phase: 'Seed', schema: SEED_SCHEMA, model: 'sonnet' })
  ))).filter(Boolean);
  ...
  const auditByDevice = await parallel([
    () => deviceChain('5560', areaPartitions['5560']),
    () => deviceChain('5554', areaPartitions['5554']),
    () => deviceChain('5562', areaPartitions['5562']),
  ]);
