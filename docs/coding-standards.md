...
**Enforce:** [Enforced] (R3, reassurance-plan.md) the 3 Story 27.14 convenience methods that previously sent `sefaria_ref`/`profile_id` are fixed: `logCompletionRecorded` now takes `curriculumId` (never `sefariaRef`); `logPinLockedOut`/`logParentModeEntered` no longer take a `profileId` parameter at all. `test/core/analytics/pv1_analytics_pii_exclusion_test.dart` is a systemic sweep — it invokes every `AnalyticsService` convenience method and asserts none of `_bannedPiiKeys` appears in its fired parameters, plus a full-catalog exhaustiveness check that fails if a new `AnalyticsEvent` member is added without being classified into this sweep or into `analytics_pv1_redaction_test.dart`'s coverage. PV-5's catalog checker (`make audit` check 26/26) remains the complementary backstop forcing every event through the reviewed `AnalyticsEvent` catalog.
