...
**Enforce:** [Enforced, scoped] `make audit` check 37/37 forbids ad-hoc mid-method `ParentAnalyticsRepositoryImpl`/`RewardMilestoneService` construction in the four sites AUD-sync-05 and AUD-core-navigation-04 fixed (`features/sync/data/local_data_upload_service.dart`, `outbox_sync_write_facade.dart`, `reward_settings_merge_delegate.dart`, `core/navigation/guards/restore_guard.dart`) — each now takes the dependency as an optional constructor/factory parameter (or, for `reward_settings_merge_delegate.dart`, a dedicated `rewardMilestoneServiceFactoryProvider`) defaulting to the prior ad-hoc construction. `restore_guard.dart` specifically takes a `ParentAnalyticsRepository Function(UserDatabase)` factory rather than a fixed instance, because the guard is a long-lived singleton whose backing `UserDatabase` can change mid-session (account switch) — the factory re-resolves against whatever `getDatabase()` returns on each check. **[Pending, backlog]** this is diff-scoped, not repo-wide: further ad-hoc `RewardMilestoneService`/similar construction sites remain (e.g. `lib/features/gamification/presentation/providers/reward_config_controller.dart`, `dashboard_providers.dart`, `achievements_overview_provider.dart`, `points_service.dart`, `child_redemption_screen.dart`) — a general `static final .*(Repository|Service|Dao)` / direct-construction-outside-provider-files grep covering all of them is future work; do not add to the scoped check's file list without also fixing the site. `make audit` check 85/85 (`tool/check_sm7_learning_program_singleton.dart`, AUD-scheduler-23) is a second, independent instance of this rule: `LearningProgramRepository.instance` is `@Deprecated('Use learningProgramRepositoryProvider via Riverpod instead')`, has zero `lib/` call sites, but 34 test-fixture call sites across 11 files still read the singleton directly instead of overriding `learningProgramRepositoryProvider` — undetected by `flutter analyze` because `deprecated_member_use_from_same_package` isn't enabled. A RATCHET (tracked baseline `tool/sm7_learning_program_singleton_baseline.txt`, currently 34) blocks growth; the migration-and-eventual-deletion backlog for the existing 34 is tracked at DNI-446.
...
**Enforce:** [Enforced, scoped to `lib/features/tracks/`] `make audit` check 85/85 (`tool/check_sm8_repo_repo_imports.dart`, AUD-tracks-15) — a `*_repository*.dart` under `lib/features/tracks/` must not import another `*_repository*.dart` (an impl importing the interface it itself implements is exempt), with no carve-outs within that scope. `TrackLearningOrderRepositoryImpl` and `LearningOrderRepositoryImpl` (both AUD-tracks-15) are the reference fix: neither holds a `ContentRepository` dependency anymore — `track_learning_order_providers.dart` / `learning_order_providers.dart` resolve the content list and pass it into `getSedarimOrder`/`getMasechtosOrder`/`getOrder` as `List<ContentItem> allItems`. Repo-wide (outside `lib/features/tracks/`) is still `[Pending, backlog]`.
...
**Why:** "which religious text a specific child studied" is sensitive personal information about a child; exporting it to Google Analytics is what Play Families / COPPA disclosure rules restrict. AUD-core-analytics-01 (2026-07) closed the larger gap: 7 call sites reached via the uncatalogued `.logEvent()` bypass (PV-5) that combined `entity_key`/`profile_id`/`target`/raw exception text with analytics events — `entity_key` alone combined a per-child id with the exact content studied in one string.
**Enforce:** [Enforced] (R3, reassurance-plan.md) the 3 Story 27.14 convenience methods that previously sent `sefaria_ref`/`profile_id` are fixed: `logCompletionRecorded` now takes `curriculumId` (never `sefariaRef`); `logPinLockedOut`/`logParentModeEntered` no longer take a `profileId` parameter at all. `InviteTutorUseCase`'s `tutor_invite_sent` (a raw `.logEvent()` call site, not a convenience method) had the same gap — a `child_profile_id` param that an earlier, exact-match version of the sweep below failed to catch — and is fixed the same way (no identifier at all). `test/core/analytics/pv1_analytics_pii_exclusion_test.dart` is a systemic sweep in two parts: (1) it invokes every `AnalyticsService` convenience method and asserts none of `_bannedPiiKeys` appears as a case-insensitive *substring* of any fired parameter key (substring, not exact match, specifically to catch `child_profile_id`-shaped keys); (2) every `AnalyticsEvent` fired only via a raw `.logEvent()` call site is exercised directly against its real production use case/service in the same file's "direct call-site sweep" group, cited as genuinely covered by another suite (verified by reading that suite), or confirmed dead code with zero `lib/` emitters. A full-catalog exhaustiveness check fails if a new `AnalyticsEvent` member is added without landing in one of these buckets. PV-5's catalog checker (`make audit` check 26/26) remains the complementary backstop forcing every event through the reviewed `AnalyticsEvent` catalog.
...
**PV-7 — Sacred Time / device location: coordinates are rounded to 3 decimal places (~111m) before persisting, and the Settings DEVICE-section location actions (Detect / Choose City) require a Parent PIN challenge when the active profile is a child with a configured PIN.**
**Why (AUD-sacred_time-08):** DEC-26 already keeps `SacredTimePreferences`' lat/long device-local and excluded from the synced `ui_preferences` payload (`outbox_sync_write_facade.dart`, "DEC-26 (WS6)" comment) — a background pull can never leak it to another profile or device, so there is **no cloud-privacy leak here**; rounding is defense-in-depth against a full-precision GPS fix sitting at rest on disk, not a sync fix. The remaining real gap: `SettingsRoute` carries no route-level PIN/child-mode guard (`app_router.dart` — `settings` has only `authGuard`, unlike the PIN-guarded `/parent-mode/*` routes), and the DEVICE section (and its `SacredTimeSettingsCard`) is shown to child profiles too, so without an in-card gate a child on a shared device could trigger GPS detection or change the city with no parent authentication. The card itself is gated instead of the whole Settings screen, which children legitimately need for their own profile preferences (Hebrew calendar, nikud, font size, etc.).
**Enforce:** [Enforced] `SacredTimePreferences.writeLocation`'s rounding is covered by `test/features/sacred_time/data/services/sacred_time_preferences_test.dart`; the Parent PIN gate (`sacredTimeLocationPinGuardRequiredProvider`, mirroring ProfileSwitcherSheet's AN-2 `switcherSheetPinGuardRequiredProvider`) is covered by `test/features/sacred_time/presentation/widgets/sacred_time_settings_card_pin_guard_test.dart`.
**Source:** `docs/_archive/superseded/entity-model-audit-2026-05-24.md` (DEC-26); `learning_tracker/lib/features/sync/data/outbox_sync_write_facade.dart`; `learning_tracker/lib/app/router/app_router.dart`.
...
**Enforce:** [Enforced] `test/helpers/pump_app.dart` (AUD-t-profiles-02) is the canonical `pumpApp({child, overrides, locale, retry, container, builder, debugShowCheckedModeBanner})` helper — `builder`/`debugShowCheckedModeBanner` (AUD-t-tutoring-03) are optional passthroughs to `MaterialApp`'s own params, both defaulting to `MaterialApp`'s defaults so every pre-existing call site is unaffected; `make audit` check 62/62 (`tool/check_tq3_pump_app_migration.dart`) is a RATCHET on the `GlobalCupertinoLocalizations.delegate` occurrence count outside `test/helpers/` — it fails only when a NEW hand-rolled delegate block raises the count above the tracked baseline (`tool/tq3_pump_app_delegate_baseline.txt`), same shape as AG-5's `check_test_mirroring.dart`. [Pending] CI golden/widget test of a key screen under Hebrew.
...
**AG-8 — Change types bind to docs: a schema/migration diff updates [data-models](data-models.md); a sync-behavior diff updates [sync-conflict-resolution](sync-conflict-resolution.md); a collection-shape diff updates [firestore-collection-layout](firestore-collection-layout.md); a schema/feature-reorg diff (table count, DAO count, or `lib/features/` module set) updates [index.md](index.md), [project-overview.md](project-overview.md), and [source-tree-analysis.md](source-tree-analysis.md) (AUD-docs-16 — these three restate DB-schema/feature-list counts and must stay consistent with `docs/architecture.md`'s generated tables); a new invariant updates this file (with its checker — Rule 0). Doc updates ship in the same PR. In docs, prefer pointers (`file:line`, symbol names) over pasted code.**
**Why:** agents trust docs; a doc that lies is worse than no doc. Pasted snippets drift the moment the code changes.
**Enforce:** [Pending] audit rule pairing diff paths with doc paths; review-checklist. For one concrete count-drift slice of this (`docs/explainers/sync-subsystem.md` restating a stale, hand-picked `EntityMerger` count instead of deriving it from `lib/core/sync/merge/`, AUD-docs-18): `dart run tool/check_sync_explainer_merger_count.dart` (learning_tracker/), wired into `make audit` (check 46/46). Not a ratchet — the doc must always state the true count; the checker fails on any mismatch, including a doc that drops the count claim entirely. **Scope note (AUD-docs-16):** `make audit` checks 48/49 (`tool/gen_arch_tables.dart --check`, `tool/gen_feature_graph.dart --check`) verify only `docs/architecture.md`'s generated "Database Schema" and "Feature Dependency Graph" sections — they do **not** read or gate `index.md`, `project-overview.md`, or `source-tree-analysis.md`. Those three docs' counts are checklist/review-enforced only (this paragraph), same as the rest of AG-8's [Pending] general case; do not describe checks 48/49 as covering them.
**Source:** anthropic.com/engineering — context engineering
...
**Enforce:** PR/commit template with a Verification section; the existing audit-wave adversarial-verify stage, kept mandatory for confirmed findings. For the story-doc slice of this (a `docs/stories/implementation/*.md` file shipping `Status: done`/`review` with no verifiable completion trail): `dart run tool/check_story_dod.dart` (learning_tracker/), wired into `make audit` (check 45/45). It is a RATCHET, not a full-repo hard-fail, the same shape as AG-5's `check_test_mirroring.dart`: the pre-existing epic-21 backlog (AUD-docs-24) is tracked in a baseline inside the script, and the gate fails only when a NEW story ships `Status: done`/`review` with an unchecked Tasks/Subtasks line or an empty Dev Agent Record — burn the backlog down incrementally, story by story.
**Source:** code.claude.com/docs — best practices
...
| 37 | SM-7 (scoped): no ad-hoc `ParentAnalyticsRepositoryImpl`/`RewardMilestoneService` construction in the four AUD-sync-05/AUD-core-navigation-04 fix sites (`local_data_upload_service.dart`, `outbox_sync_write_facade.dart`, `reward_settings_merge_delegate.dart`, `restore_guard.dart`) — see SM-7 above for the wider repo-wide backlog |
