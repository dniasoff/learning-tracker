...
**Enforce:** [Retired with the deleted Drift/outbox sync engine]. The former core/sync EH-4 check targeted periodic-drain, recovery-pull, and listener-dispatch files that no longer exist. Remaining EH-4 scopes retain their own live checkers.
...
**Enforce:** [Retired with the Drift/outbox sync engine]. The former `push*` gateway check 28 was specific to `FirestoreGatewayImpl` and its server-timestamp overwrite contract. The Firestore-native repositories now distinguish client `updated_at` ordering from server `synced_at` acknowledgement metadata; the old gateway check was removed with the deleted engine rather than redirected to a semantically different write path.
...
| `avoid_catches_without_on_clauses` (project-wide) | EH-4 — old core/sync scope retired; remaining scopes use live checkers |
| `avoid_catching_errors` (project-wide) | EH-4 — old core/sync scope retired; remaining scopes use live checkers |
...
| 37 | SM-7: no ad-hoc `ParentAnalyticsRepositoryImpl` construction anywhere in `lib/` outside `core/analytics/` — `tool/check_sm7_analytics_repository_construction.dart` |
