...
| 73 | Repo hygiene (ratchet): no NEW stray `.gitkeep` in a `lib/` directory that already contains another git-tracked file — `tool/check_gitkeep_stray.dart` (AUD-core-constants-02). A `.gitkeep` exists only to make git track an otherwise-empty directory; 47 pre-existing offenders (discovered by this checker's own first run, out of this finding's named scope) are tracked in an inline baseline and tolerated — only a NEW stray `.gitkeep` outside that baseline, or `lib/core/constants` regressing, fails the gate. |
