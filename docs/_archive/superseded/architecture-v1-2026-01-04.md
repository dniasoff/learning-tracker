...
**Continuous Integration (Local Pipeline):**
- **Strategy:** Local CI via Makefile + Git hooks (no cloud CI services)
- **Pipeline Orchestration:** Makefile with standardized targets (`make lint`, `make test`, `make build`, `make ci`)
- **Automated Triggers:** Git pre-push hook runs `make ci` to validate before pushing
- **Build Validation:** Linting (`dart analyze`), formatting (`dart format --set-exit-if-changed .`), build success (`flutter build apk`)
- **Test Execution:** Unit tests, widget tests (`flutter test`), integration tests on headless emulator (`flutter test integration_test/`)
- **Code Generation Check:** `dart run build_runner build --delete-conflicting-outputs` as first CI step
- **APK Build:** `flutter build apk --release --dart-define-from-file=config/prod.json`
- **Affects:** Development workflow, deployment process
- **Rationale:** Solo developer on dedicated server — local pipeline provides identical guarantees to cloud CI with zero external dependencies, no network latency, and no service costs

### Development Environment & Remote Access

**Server Specification:**
- **Hardware:** Dedicated Linux server, 64GB RAM
- **OS:** Linux (Ubuntu/Debian-based)
- **Physical Device Access:** None — all development and testing via Android emulator
- **Rationale:** 64GB RAM provides ample headroom for simultaneous Flutter builds, Android emulator (4-8GB allocation), Firebase Emulator Suite, and development tooling

**Android Emulator Setup (KVM-Accelerated):**
- **Virtualization:** KVM hardware acceleration required (`/dev/kvm` must be accessible)
- **System Image:** x86_64 Android API 34 (Google APIs) for near-native emulator performance
- **Emulator Allocation:** 4-8GB RAM, 4GB internal storage
- **GPU Rendering:** `swiftshader_indirect` for software GPU rendering on headless server (no physical GPU required)
- **Creation Command:**
  ```bash
  ...
  # Create AVD
  avdmanager create avd -n mishnayos_dev -k "system-images;android-34;google_apis;x86_64" -d pixel_6

  # Launch for interactive use (with display)
  emulator -avd mishnayos_dev -gpu swiftshader_indirect

  # Launch for CI/headless testing (no display)
  emulator -avd mishnayos_dev -no-window -no-audio -gpu swiftshader_indirect
  ```

**Remote Access Methods:**

**1. Code Editing — VS Code Remote SSH (Primary):**
- **Tool:** VS Code with Remote - SSH extension
- **Usage:** Connect VS Code locally to the Linux server; edit files, run terminal commands, use Flutter/Dart extensions — all remotely
- **Setup:** SSH key authentication to server, install Flutter/Dart VS Code extensions on remote
- **Advantage:** Full IDE experience with zero latency on code editing; all computation runs on server

**2. Emulator Interaction — scrcpy over Network (Primary):**
- **Tool:** scrcpy (screen copy) — lightweight Android device/emulator mirroring
- **Setup:**
  ```bash
  # On server: start emulator and expose ADB over TCP
  emulator -avd mishnayos_dev -gpu swiftshader_indirect &
  adb tcpip 5555

  # On local machine: connect to server's ADB and mirror
  adb connect <server-ip>:5555
  scrcpy --serial <server-ip>:5555
  ```
- **Performance:** Near-native framerate, low latency input — suitable for interactive testing, verifying animations, checking RTL layout
- **Alternative (SSH tunnel):** If server is not on local network, tunnel ADB over SSH:
  ```bash
...
**3. Browser-Based Fallback — noVNC:**
- **Tool:** noVNC (VNC client in browser) + TigerVNC server
- **Usage:** Access full Linux desktop (including emulator window) from any browser
- **Setup:**
  ```bash
  # On server: start VNC server and noVNC websocket proxy
  vncserver :1 -geometry 1920x1080
  websockify --web /usr/share/novnc 6080 localhost:5901
  ```
- **Access:** Browse to `http://<server-ip>:6080` from any device
- **Use Case:** When scrcpy is not available on the local machine, or for quick mobile browser access

**Firebase Emulator Suite (Local Development):**
- **Tool:** Firebase Emulator Suite (Authentication + Firestore)
- **Purpose:** Local Firebase services for development — no cloud dependency, no quota limits, reproducible state
- **Setup:**
  ```bash
  ...
  firebase init emulators  # Select: Authentication, Firestore

  # Start emulators
  firebase emulators:start
  ```
- **Emulator UI:** Web dashboard at `http://localhost:4000` for inspecting Firestore data and auth state
- **Flutter Integration:** Point Firebase SDK to local emulators in development config:
  ```dart
  // In dev configuration only
  FirebaseFirestore.instance.useFirestoreEmulator('localhost', 8080);
  FirebaseAuth.instance.useAuthEmulator('localhost', 9099);
  ```
- **Affects:** All Firebase-dependent development and testing; switch to live Firebase only for final integration validation

**Local CI Pipeline:**

**Makefile Targets:**
```makefile
.PHONY: codegen lint format test test-integration build ci clean
...
ci: codegen lint format test build
...
#!/bin/sh
# .git/hooks/pre-push
echo "Running CI pipeline before push..."
make ci
if [ $? -ne 0 ]; then
...
| Test Layer | Tool | Device Required | Coverage Target |
|---|---|---|---|
| Unit Tests | `flutter test` | None | 80%+ business logic |
| Widget Tests | `flutter test` | None | Critical UI components |
| Integration Tests | `flutter test integration_test/` | Headless emulator | Core user flows |
| Visual Verification | scrcpy + emulator | Remote emulator | RTL layout, animations, MD3 theme |

**What Can Be Fully Validated on Emulator:**
- All UI rendering including Hebrew RTL layout
- SQLite/drift database operations and transactions
- Firebase operations (via Firebase Emulator Suite)
- Navigation flows, state management, onboarding
- Completion animations, progress bars, streak UI
- Parent/tutor PIN access control
- Offline-first behavior (emulator network toggle)
- Smart scheduler calculations and daily recommendations

**What Requires Deferred Physical Device Testing:**
- True 60fps performance benchmarking on mid-range hardware (NFR5-7)
- Real battery drain measurement from background sync (NFR8)
- Physical push notification UX on Android 13+ (emulator approximates)
- Real offline/online network transitions (emulator simulates, not identical)
- Sub-2-second cold startup on real hardware (NFR1)

**Mitigation:** Design for performance throughout development. Defer final performance benchmarking to a brief physical device testing session before handing APK to Yisroel Meir. 95%+ of development and testing is viable on the Linux server.
...
├── Makefile                      # Local CI pipeline targets
├── firebase.json                 # Firebase Emulator Suite config
├── .firebaserc                   # Firebase project alias
├── config/
│   ├── dev.json                  # Development Firebase config (emulator endpoints)
│   └── prod.json                 # Production Firebase config (live endpoints)
├── android/
│   ├── app/
│   │   ├── build.gradle
│   │   └── src/
...
**CI/CD Integration:**
- Local pipeline: `Makefile` with `make ci` (codegen → lint → format → test → build)
- Git pre-push hook: Runs `make ci` automatically before push
- Validation: code generation, linting, formatting, tests
- Build: Android APK for distribution (`make build`)
