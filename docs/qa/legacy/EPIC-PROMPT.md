...
A comprehensive manual testing suite has been created at `docs/qa/legacy/` containing 18 documents with ~500 test scenarios organized by feature area. A developer needs to work through these documents systematically, testing every scenario and filing bugs in Linear for anything that fails.
...
Read the existing epics at `docs/planning/epics.md` to match the format and numbering conventions.
The PRD is at `docs/planning/prd.md` and the architecture at `docs/planning/architecture.md`.
