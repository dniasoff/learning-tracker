...
> ⚠️ **SUPERSEDED — 2026-04-19.** This spec describes a "Backup & Sync" Settings flow (local UUID → Firebase UID migration, magic-link auth) that was **never fully built** in this shape. Epic 23 (DNI-223) shipped a different model: every user has a real email+password account from the start, and local-born accounts upgrade to cloud-born via an "Upgrade to Cloud" flow (`UpgradeToCloudRoute`) per v2 spec §4.3.
>
> The local UUID → Firebase UID migration described below never runs — the v2 schema has no `localUid` column.
>
> **Current canonical spec:** [`../../planning/ux-upgrade-flow-spec.md`](../../planning/ux-upgrade-flow-spec.md) and [`../../planning/architecture-offline-v2.md`](../../planning/architecture-offline-v2.md) §4.3. See also [DNI-188](https://linear.app/dniasoff/issue/DNI-188) for the Linear ticket with a superseded banner.
...
This story implements Phase 6 of the prior local-first auth abstraction design (now superseded by `docs/planning/architecture-offline-v2.md`). It assumes the following prior stories are complete:
...
- [Source: `docs/planning/architecture-offline-v2.md` — v2 supersedes the prior UID migration and deferred-account-creation design; this story's implementation reflects the prior design and is tracked for refactor in the v2 refactor epic]
