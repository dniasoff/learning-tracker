...
> ⚠️ **SUPERSEDED — 2026-04-19.** This spec describes the March 2026 anonymous-`localUid` auth model (sealed `AppAuthState`, `LocalAuthGuard`, nullable `firebaseUid`, `hasAccount` flag). That model was implemented, then **immediately superseded** by Epic 23 (DNI-223) hard-tier cloud-born/local-born refactor on 2026-04-15. None of the columns, classes, or guards described below exist in the current codebase:
>
> - `localUid` column → replaced by `email` + `passwordHash` + `tier`
> - `AppAuthState` / `LocalAuthState` / `CloudAuthState` sealed hierarchy → replaced by unified `AuthState`
> - `LocalAuthGuard` → collapsed into single `AuthGuard`
> - `hasAccount` flag → removed; every user has a real account
>
> **Current canonical spec:** [`../../planning/architecture-offline-v2.md`](../../planning/architecture-offline-v2.md). See also [DNI-186](https://linear.app/dniasoff/issue/DNI-186) for the Linear ticket with a superseded banner.
...
- [Source: docs/planning/architecture-offline-v2.md — v2 supersedes the prior local-first auth abstraction design; this story's implementation reflects the prior design and is tracked for refactor in the v2 refactor epic]
