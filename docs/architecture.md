...
> **AUD-docs-15/16 regenerated 2026-08-18** by `tool/gen_arch_tables.dart` (`make gen-arch-tables` to reprint; `make check-arch-tables` fails CI if this section drifts from a fresh run). The former "User DB" (`lib/core/database/user/user_database.dart`, 24 profile-scoped tables) was archived wholesale in the Firestore-native rewrite (Phase 3 P3-5) — every table it held is now a Firestore document under `lib/data/firestore/**`/`lib/data/repositories/**`, not a local Drift table. `tool/gen_arch_tables.dart`'s User DB spec was removed in the same commit that fixed `make audit`'s crash on the now-deleted `user_database.dart` path. Two local Drift databases remain, both genuinely device-local (not profile-scoped user data): the content cache and the device registry.
...
| Registry DB (`device_registry.db`) | **v2** | 2 | — (inline) |
...
| Registry DB | DeviceAccounts | 11 |
| Registry DB | DeviceState | 2 |

_Generated by `tool/gen_arch_tables.dart`. Total: 4 Content DB + 2 Registry DB = 6 tables._

### Key Schema Notes

- **Profile-scoped data lives in Firestore, not Drift:** every table this section used to list under "User DB" (accounts, profiles, tracks, completions, goals, etc.) is now a Firestore document tree under `lib/data/firestore/**`/`lib/data/repositories/**`. `profileId`-equivalent scoping is enforced by `tool/check_profile_path_keying.dart` (`make audit` check 103, PROFILE-KEY-SPLIT) against `firestore.rules`, not by a Drift primary-key checker.
- **Cascade deletes:** now a Cloud Functions concern (`functions/src/deletes.ts`) rather than Drift's declarative cascade.
- **Content DB**: the on-device Torah-text cache — genuinely device-local, downloaded once and shared across every profile on the device, not synced.
- **Registry DB**: the device-account registry (`lib/core/database/registry/`) — tracks which accounts are known to this device and their persisted Firestore-path uid (see `PathUidResolver`, AD-24/AD-19). Also device-local by design; it is what lets the app resolve which account is active before any network round-trip.
