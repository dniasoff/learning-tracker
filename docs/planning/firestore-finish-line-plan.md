...
**Status:** COMPLETE (2026-08-18, HEAD `bf283479`). All 6 §1 done-criteria verified against code, `make audit`, and an exhaustive serialized `flutter test` run (see §7). This plan is retained as the historical record of how completion was reached.
...
**Criterion 6 closed out, 2026-08-18 (HEAD `6d12598e`), with a caveat about how it was measured.** A default-concurrency `flutter test --coverage` run on this host can silently never schedule entire test files under memory pressure — exit 0, "All tests passed!", zero error output — while claiming a clean result (see memory `flutter-test-concurrent-silent-file-drop`, and `docs/test-artifacts/firestore-finish-line-baseline-failfiles.txt`'s own closeout commit `1fa2ebd0`, whose full-suite run turned out to have only genuinely executed 150/783 files). Closed properly in two steps: (1) the 26-file failing baseline from `ca984f43` was independently re-verified with `flutter test -j 1` (serialized, immune to the drop) — 359/359 passed, all 26 files confirmed present in the log. (2) The remaining 633 files absent from that concurrent run's log were serialized in two batches — a 149-file risk-weighted sample (all of `test/data`, `test/integration`, `test/e2e`, `test/app`, `test/l10n`, plus stratified spreads of `test/features`, `test/core`, `test/story_acceptance`, `test/tool`), then the remaining 484 — both confirmed 100% present via byte-level log verification, both **0 failures** (1670 passed/22 skipped in 2h15m; 4307 passed/29 skipped in 8h08m). Combined with the 150 files already confirmed present-and-passing in the original concurrent run, all 783 test files in the suite now have serialized-or-confirmed-present execution evidence with zero failures. `flutter test` genuinely shows no regression against the recorded baseline.
...
| L8 — D-E round two | **Landed, verified** | Dispatched and mostly completed by an earlier interrupted session (found mid-flight, uncommitted, at this round's start — recovered per the "session died mid-build" case, not re-dispatched from scratch). This round finished the compile-error cleanup: `data_export_import_service.dart`'s `FirebaseFirestore` boundary removal (item 14) completed via a new `BackupFirestoreGateway`/`_DynamicBackupFirestoreGateway` split so the domain service no longer imports `cloud_firestore` directly; the other 8 files' D-E-round-two changes (item 13) compiled clean as-is. Landed in `df53efa2`. Verified by orchestrator: `dart analyze --fatal-infos` 0 errors in `lib/` (was 26 at round start, all traced to this same unfinished work). No full-suite per-lane number recorded — see the Phase-1.5 note below; the full-suite gate run this round measures the whole tree, not L8 in isolation. |
| Phase 2 — Int-identity | **Landed, verified (2026-08-17, `82bda77c`), via a different route than planned** | Never dispatched as the planned A/B/C sub-phases below (stale — left unedited as the historical record of what was scoped). Instead, a direct audit of every remaining `kGlobalTrackSentinel`/`profileId:0`/`trackId:0` site against this plan's AD-24/AD-25 definition of done found most of it was already-dead Drift-era debt (deleted outright: `GoalRepository.createGoal`'s int params, `StreakLogEvent.profileId`, `TrackRepository.initializeDefaultTracks`'s unused param, a dead `trackId==0` bulk-mark branch, `kFirestoreUnmappedCompletionRowId`, a dead `_legacyProfileId` constant) and `RewardMilestone.kGlobalTrackSentinel` itself is legitimate design (a real "applies to all tracks" concept `NextRewardSelector` still needs), not fabricated identity — left in place. 2026-08-18 re-verification found zero `profileId: 0`/`trackId: 0` call sites in `lib/`, and `kFirestoreUnmappedStageId` is confirmed fully deleted (one stale doc-comment cross-reference remains at `completion_entity.dart:23`, harmless). One residual, non-blocking finding: `AchievementRowVm.trackId`/`AchievementTrackFilterVm.trackId` in the achievements screen are always the sentinel (never a live bug — `achievements_overview_provider.dart` has had exactly one construction path since DEC-32/GA-3 removed per-track rewards) — vestigial dead code, not fixed, flagged for a future pass. *(Original A/B/C split, superseded: A (goal/track identity, 18 files), B (stage identity, 15 files), C (profile/streak identity + residual sentinels, 12 files).)* |
| Phase 3 — Test repair | **Done** — see §7's 2026-08-18 criterion-6 closeout | Per §4. |
