...
> ⚠️ **SUPERSEDED — 2026-04-19 review.** This PRD (v2, dated 2026-02-08) documents the v1 scope **as of February 2026**. Several foundational claims no longer match the shipped product as of 2026-04-15:
>
> - **Curricula:** Describes 5 curricula. Shipped: **9** (added Nach, Tanach, Mussar, Mishneh Torah).
> - **Track model:** Describes three parallel tracks (Personal + School + Tutor) as a core capability. Shipped: `personal` is the default; `school` and `tutor` ship in code but are not promoted in v1 onboarding.
> - **Auth model:** Not explicitly defined here; the shipped model is **hard-tier** (Epic 23): every user has an email+password account, tier determined at signup.
> - **Databases:** No architecture detail here; shipped is a three-database split (Epic 19) — User DB v4, Content DB v3, Device Registry DB v1.
>
> This PRD remains the canonical *original product requirements*. For current behavior, trust: [`../project-overview.md`](../project-overview.md), [`architecture-design.md`](architecture-design.md), [`architecture-offline-v2.md`](architecture-offline-v2.md), [`epics.md`](epics.md).
