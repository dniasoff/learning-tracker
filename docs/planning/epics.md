...
### Epic 10 — Parent Mode (DNI-14, 6 stories)
...
### Epic 11 — Tutor Mode (DNI-15, 4 stories)
...
### Epic 12 — Notifications (DNI-16, 3 stories)
...
### Epic 13 — Cloud Sync (DNI-17, 3 stories)
...
### Epic 14 — Settings (DNI-18, 4 stories)
...
### Epic 18 — Onboarding & Track Management Overhaul (DNI-128, 8 stories)
...
- **DNI-180** — 18.1: Add Track Flow — 8 Screens, One Concept Each *(In)*
...
### Epic 21 — Multi-Account Device — Account Switching, Session Management & Deletion (DNI-238, 0 stories)


### Epic 22 — Catch-up & Amnesty System (DNI-255, 22 stories)

- **DNI-256** — 22.1: item_amnesty Table + DAO *(Backlog)*
- **DNI-257** — 22.2: curriculum_tracks Per-Track Settings + Pause Columns *(Backlog)*
- **DNI-258** — 22.3: TrackDebt Computed Struct + Scenario Detection *(Backlog)*
- **DNI-259** — 22.4: track_action_log Table + DAO *(Backlog)*
- **DNI-260** — 22.5: Rescope Service Wrapper *(Backlog)*
- **DNI-261** — 22.6: Amnesty Primitive — Create, Revoke, Undo *(Backlog)*
- **DNI-262** — 22.7: Amnesty History View *(Backlog)*
- **DNI-263** — 22.8: Pause Track State + PausePicker *(Backlog)*
- **DNI-264** — 22.9: Pause Auto-Resume + PauseResumeCard *(Backlog)*
- **DNI-265** — 22.10: Catch-up Sheet — Self-Paced Variants (S1, S2, S3) *(Backlog)*
- **DNI-266** — 22.11: Catch-up Sheet — Program Variant (S4) *(Backlog)*
- **DNI-267** — 22.12: Review Debt View (S6, S7) *(Backlog)*
- **DNI-268** — 22.13: Learning Journey Structural View (S8) *(Backlog)*
- **DNI-269** — 22.14: Triage Sheet (S9, S11) *(Backlog)*
- **DNI-270** — 22.15: Returning Learner Onboarding (S9) *(Backlog)*
- **DNI-271** — 22.16: Cross-Credit Opt-In (S12) *(Backlog)*
- **DNI-272** — 22.17: Siyum Cleanup Card (S13) *(Backlog)*
- **DNI-273** — 22.18: Setup Seeding — Program Launch Day (S14) *(Backlog)*
- **DNI-274** — 22.19: Setup Seeding — Personal Track Retrofit (S15) *(Backlog)*
- **DNI-275** — 22.20: Cycle-Boundary Welcome Flow *(Backlog)*
- **DNI-276** — 22.21: Notification System Rewrite — Pause-Based Silence *(Backlog)*
- **DNI-277** — 22.22: Track Settings Panel — Recovery Configuration *(Backlog)*

### Epic 23 — Manual QA Verification (DNI-278, 18 stories)

- **DNI-279** — 23.2: Set Up Testing Environments *(Backlog)*
- **DNI-280** — 23.1: Read QA Methodology & Product Overview *(Backlog)*
- **DNI-281** — 23.3: Test Auth & Accounts *(Backlog)*
- **DNI-282** — 23.4: Test Onboarding Flow *(Backlog)*
- **DNI-283** — 23.6: Test Multi-Track Learning *(Backlog)*
- **DNI-284** — 23.5: Test Learning & Completions (CRITICAL) *(Backlog)*
- **DNI-285** — 23.7: Test Configurable Stages & Learning Order *(Backlog)*
- **DNI-286** — 23.8: Test Scheduler & Goals *(Backlog)*
- **DNI-287** — 23.9: Test Dashboard & Progress *(Backlog)*
- **DNI-288** — 23.10: Test Content Browsing & Search *(Backlog)*
- **DNI-289** — 23.11: Test Gamification *(Backlog)*
- **DNI-290** — 23.12: Test Parent Mode *(Backlog)*
- **DNI-291** — 23.13: Test Tutor Mode *(Backlog)*
- **DNI-292** — 23.14: Test Notifications *(Backlog)*
- **DNI-293** — 23.15: Test Settings *(Backlog)*
- **DNI-294** — 23.17: Test Profiles *(Backlog)*
- **DNI-295** — 23.16: Test Sync & Offline *(Backlog)*
- **DNI-296** — 23.18: Test Catch-up & Amnesty System (BLOCKED) *(Backlog)*

### Epic 24 — Firestore Sync Schema & Multi-Device Data Restoration (DNI-297, 10 stories)

- **DNI-298** — 24.1: Firestore Document Schema & Collection Design *(Backlog)*
- **DNI-299** — 24.2: Profile & Account Document Sync *(Backlog)*
- **DNI-300** — 24.3: Track Setup Collections Sync (6 tables) *(Backlog)*
- **DNI-301** — 24.4: Learning Configuration Sync (4 tables) *(Backlog)*
- **DNI-302** — 24.5: Completions & Learning Ledger Sync — Append-Only (2 tables) *(Backlog)*
- **DNI-303** — 24.6: Gamification State Sync — Streaks & XP Events (3 tables) *(Backlog)*
- **DNI-304** — 24.7: Rewards & Pools Sync (3 tables) *(Backlog)*
- **DNI-305** — 24.8: Test Scores & Account Settings Sync (1 table + account doc) *(Backlog)*
- **DNI-306** — 24.9: New Device Full Restore (Pull-on-Launch) *(Backlog)*
- **DNI-307** — 24.10: Local-Born Upgrade Push & Ongoing Sync Engine *(Backlog)*
...
- **DNI-322** — 25.1: Schema-v1 user DB skeleton (renamed tables, profileId PKs, no defaults, FKs) *(Backlog)*
- **DNI-323** — 25.2: Append-only event tables with composite-natural-key UNIQUEs *(Backlog)*
- **DNI-324** — 25.3: Composite indexes on hot-path queries *(Backlog)*
- **DNI-325** — 25.4: Firestore v1 collection layout and per-collection rules *(Backlog)*
- **DNI-326** — 25.5: Outbox table and OutboxProcessor scaffolding *(Backlog)*
- **DNI-327** — 25.6: Schema-check tool to enforce profileId-in-PK and composite-index invariants *(Backlog)*
- **DNI-328** — 25.7: core/preferences/ — six ProfileScopedPreference primitives *(Backlog)*
- **DNI-329** — 25.8: core/content/ContentIndex — indexed lookup for 9 curricula + ProgramRefResolver *(Backlog)*
- **DNI-330** — 25.9: core/labels/ rebuild — three new modes, ContentIndex consumer, static API deleted *(Backlog)*
- **DNI-331** — 25.10: core/time/LocalDayClock — single time provider *(Backlog)*
- **DNI-332** — 25.11: core/auth/AuthRepository — sole Firebase Auth consumer *(Backlog)*
- **DNI-333** — 25.12: SyncEngine decomposition Part 1 — FirestoreGateway, PushPipeline, PullPipeline *(Backlog)*
- **DNI-334** — 25.13: SyncEngine decomposition Part 2 — MergeRouter and sealed EntityMerger strategies *(Backlog)*
- **DNI-335** — 25.14: SyncEngine decomposition Part 3 — ListenerSupervisor and LifecycleObserver *(Backlog)*
- **DNI-336** — 25.15: core/learning/CompletionWriter — single transactional commit path *(Backlog)*
- **DNI-337** — 25.16: core/streak/ — event log + reducer + round-trip sync *(Backlog)*
- **DNI-338** — 25.17: core/database/BaseDao&lt;T&gt; and TrackScope; delete cross-profile DAO methods *(Backlog)*
- **DNI-339** — 25.18: core/navigation/ — typed auto_route + PinScope-parameterized guard *(Backlog)*
- **DNI-340** — 25.19: core/logging/ — finalize structured AppLogger and migrate remaining production logs *(Backlog)*
- **DNI-341** — 25.20: MaterialApp locale auto-detection + Noto Sans Hebrew bundling + direction-aware CurriculumLabel + real dark theme *(Backlog)*
- **DNI-342** — 25.21: Multi-account threading — replace eight hardcoded currentAccountId = 1 sites + offline indicator preservation *(Backlog)*
- **DNI-343** — 25.22: Wipe-install cutover end-to-end verification *(Backlog)*

### Epic 26 — Feature Rebuilds + Cleanups (Phases 3 + 4) (DNI-314, 35 stories)

- **DNI-308** — 26.34: Delete deprecated TextDownloadService *(Backlog)*
- **DNI-309** — 26.35: Remove promoteToCloud / demoteToLocal auth shims *(Backlog)*
- **DNI-344** — 26.1: Scheduler strategy pattern — SchedulerInput → SchedulerAnalysis → TaskAssembly *(Backlog)*
- **DNI-345** — 26.2: Fix dashboardPaceStatusProvider with real total-items math *(Backlog)*
- **DNI-346** — 26.3: Scheduler classification + chazara-load + isStudyDay + day-1 rolling-window fixes *(Backlog)*
- **DNI-347** — 26.4: GoalEntity replaces GoalFormResult; sealed PaceTarget; typed PaceGranularity *(Backlog)*
- **DNI-348** — 26.5: Extract 20 private classes from dashboard_screen.dart into widgets/ *(Backlog)*
- **DNI-349** — 26.6: TrackCard + 5 subcomponents + TrackCardViewModel *(Backlog)*
- **DNI-350** — 26.7: dashboardModelProvider composition; centralized after-track-change invalidation *(Backlog)*
- **DNI-351** — 26.8: Delete TrackProgressVariant and supporting dead code *(Backlog)*
- **DNI-352** — 26.9: AddTrackController state machine + AddTrackFlowScreen shell *(Backlog)*
- **DNI-353** — 26.10: Decompose AddTrackFlow steps into per-step files *(Backlog)*
- **DNI-354** — 26.11: OnboardingController + OnboardingStep list pattern *(Backlog)*
- **DNI-355** — 26.12: ProfileCreationUseCase (one transactional) *(Backlog)*
- **DNI-356** — 26.13: Reader purity — pure render, CompletionWriter, completionCommittedProvider *(Backlog)*
- **DNI-357** — 26.14: ContentTree indexed lookup replaces 4-level _currentLevel ladders *(Backlog)*
- **DNI-358** — 26.15: CompositeCurriculumStrategy + transactional saveOrder + parent-control at repository *(Backlog)*
- **DNI-359** — 26.16: Tappable Progress overview stats + StatCard primitive *(Backlog)*
- **DNI-360** — 26.17: StreakCalendar honors startDate/endDate; StreakHistoryScreen created *(Backlog)*
- **DNI-361** — 26.18: Lifetime providers split — per-curriculum lazy family + collapsed summaries *(Backlog)*
- **DNI-362** — 26.19: UnitCompletion model + achievementsOverviewProvider autoDispose *(Backlog)*
- **DNI-363** — 26.20: PreferenceListTile + PreferenceSegmentedTile primitives *(Backlog)*
- **DNI-364** — 26.21: PinFlowController + PinFlowScreen + PinFlowMode (3 PIN screens → 1) *(Backlog)*
- **DNI-365** — 26.22: Shared TrackManagementBody + curriculum-minimum-1 guard *(Backlog)*
- **DNI-366** — 26.23: Data export rewrite — all 23 tables, profileId on every row, no PII, round-trip test *(Backlog)*
- **DNI-367** — 26.24: Sacred-time-aware notifications — rolling 14-day batch + fire-time check + SacredWindow persistence *(Backlog)*
- **DNI-368** — 26.25: SacredTimeLockOverlay scoped to post-auth shell *(Backlog)*
- **DNI-369** — 26.26: Stage repository as only path; full params; transactional reorder + Learn-at-1 guard *(Backlog)*
- **DNI-370** — 26.27: Bulk-mark-prior streak abstention at all stages *(Backlog)*
- **DNI-371** — 26.28: Label bypass elimination — 17 files + TrackType + Calendar/LearningProgram + locale-named locals *(Backlog)*
- **DNI-372** — 26.29: Hardcoded strings → ARB extraction *(Backlog)*
- **DNI-373** — 26.30: Hebrew ARB translation completion *(Backlog)*
- **DNI-374** — 26.31: RTL widget audit — direction-aware variants across ~80–100 sites *(Backlog)*
- **DNI-375** — 26.32: Naming sweep — unit→3 names, Profiles→Accounts/LearnerProfiles, Gregorian→English, notification taxonomy *(Backlog)*
- **DNI-376** — 26.33: Dead code purge — ≥10 000 LOC across reducers, services, tables, widgets, ARB keys, themes, network modules *(Backlog)*

### Epic 27 — Discipline & Closure (Phases 5 + 6 + 7) (DNI-315, 17 stories)

- **DNI-127** — 27.17: CI/CD — Play Store deployment pipeline *(Canceled)*
- **DNI-377** — 27.1: Test infrastructure — fake_cloud_firestore, golden scaffolding, real-Drift in-memory helper *(Backlog)*
- **DNI-378** — 27.2: Unit test suite for pure functions *(Backlog)*
- **DNI-379** — 27.3: DAO and repository test suite using real in-memory Drift *(Backlog)*
- **DNI-380** — 27.4: Widget + golden test suite (canonical screens) with Hebrew variants *(Backlog)*
- **DNI-381** — 27.5: Integration test — bulk_mark_prior_does_not_credit_streak (any stage) *(Backlog)*
- **DNI-382** — 27.6: Integration tests — streak_reducer_reconciles + cloud_restore_preserves_streak *(Backlog)*
- **DNI-383** — 27.7: Integration tests — multi_profile_isolation + track_card_canonical_layout *(Backlog)*
- **DNI-384** — 27.8: Integration tests — firestore_rules (emulator) + offline_completion_flushes *(Backlog)*
- **DNI-385** — 27.9: Integration tests — pin_lockout_cycle + log_redaction + bookmark_advance_atomic *(Backlog)*
- **DNI-386** — 27.10: Custom lints Part 1 — no-curriculum-display-name-bypass, no-feature-cross-import *(Backlog)*
- **DNI-387** — 27.11: Custom lints Part 2 — no-firebase-outside-core, no-raw-talker, RTL discipline *(Backlog)*
- **DNI-388** — 27.12: CI matrix — analyze, format, audit, lint, test, coverage-floor, firestore-rules, golden, arb-parity *(Backlog)*
- **DNI-389** — 27.13: `make audit` Makefile target + tool/arb_parity_check.dart *(Backlog)*
- **DNI-390** — 27.14: 12 analytics events wired + Crashlytics user ID set everywhere *(Backlog)*
- **DNI-391** — 27.15: `docs/architecture.md` rewrite to match rebuild reality *(Backlog)*
- **DNI-392** — 27.16: CLAUDE.md and docs/coding-standards.md updated with layering rules and enforcement greps *(Backlog)*
<!-- AUTO:EPIC-LIST-END -->
