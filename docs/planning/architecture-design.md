...
> ⚠️ **Status — 2026-04-19.** This doc is a **layered record** of architectural decisions. Some decisions are canonical and current (D1, D3, D4, D5, D6, D7, D8 — all still match shipped code). **D2 (authentication) has been superseded twice**: D2 → D2-v2 (Epic 19 offline-first) → **D2-v3 (Epic 23, shipped 2026-04-15)**, the current hard-tier cloud-born/local-born model. Where this doc references "Epic 20" in connection with the hard-tier auth refactor, read it as **Epic 23** — the refactor was originally filed in Linear as Epic 20 and renamed on 2026-04-19 to resolve a numbering collision with DNI-210 (Dashboard & Progress Redesign). For the canonical auth spec, see [`architecture-offline-v2.md`](architecture-offline-v2.md).
