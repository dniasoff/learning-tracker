...
**Risk register.** (a) Named-app teardown leaks native resources / cache files → OOM (note the `api29-learn-oom` thread) — bound app count, verify disposal. (b) Anonymous-Auth-per-local-born (AD-19 `[ASSUMPTION]`) unconfirmed → local-born accounts have no principal; **blocks Phase 4** if not decided. (c) Per-account cache disk cost ×5 → set bounded `cacheSizeBytes`.
...
**Risk register.** (a) The one-time key re-home mis-maps a key → a user loses a setting/PIN → make it additive-then-verify, never destructive. (b) AD-19/20/21 unconfirmed → this phase stalls; surface early. (c) Anonymous-Auth quota/limits per project.
...
2. **Local-born (credential-less) tier has no clean Firebase principal.** If Anonymous Auth (AD-19) is rejected and no substitute is found, the offline-first account model has a hole exactly where the app's onboarding lives.
