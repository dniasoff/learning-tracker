...
| **R8 Memory/perf on low-end devices (NEW, from run-8)** | 🟠 (Part A landed) | dev (merged, ci gating) | **Part A DONE + red-demoed:** items-learned aggregates now materialize ONLY curricula with completions (1-track user: 1 not 9) — fixes the common Learn-tab crash. Added `countLeavesForCurriculum` (transient, non-caching) + equivalence test. **Part B DEFERRED (correctly):** the header "X / 70,033" denominator = `\|union(all leaf refs)\|` (70,033) ≠ per-curriculum count-sum (93,395; Chumash/Nach ⊂ Tanach) — a naive count-sum would change the total, so the union-aware count path is a separate follow-up. Until Part B, the Dashboard/Lifetime-Knowledge denominator still loads all 9 → residual OOM risk there. **UPDATE — Part B LANDED (merge `5db8634c`):** `lifetimeTotalsAcrossAllCurriculaProvider` now computes both totals without a full tree per curriculum, via a new leaf-only non-caching `loadLeavesTransient` behind a separate `LifetimeUnionLeafSource` capability (kept off `ContentRepository` so ~60 existing test doubles are untouched). The union denominator is still exactly **70,033** on real assets — pinned as an invariant, and subset-bridging (Chumash/Nach ⊂ Tanach) proven sound by a 6-scenario equivalence test against the untouched old provider rather than merely asserted. Red-demo: forcing the old path reddened the guard, listing all 9 materialized curricula. **⚠️ CORRECTION — what the device actually confirms.** Two devices read **70,033** (5554: 4,192/70,033 after bulk-marking 6 sedarim; 5558: 70,033 on the Lifetime Knowledge header). I first wrote that this "confirmed Part B", implying an unfixed build would have shown 93,395. **That was wrong.** 5558 deliberately measured *both* a pre-Part-B build (installed 16:27) and a post-Part-B build (containing `5db8634c`) and saw **70,033 on both** — it never saw 93,395 at all. **Part B is a memory/caching fix, not a miscount fix: the displayed number was already correct.** So the device evidence confirms Part B caused **no regression** in the total, and confirms the 70,033 invariant — it does *not* demonstrate a user-visible defect that Part B repaired. **Both OOM paths bounded.** **UPDATE — run-10 SETTLES IT: run-8's P0 is REFUTED.** Two runs failed to resolve this; run-10 closed it with numbers, on API 29 with the eager load still present and confirmed unfixed (`content_index.dart:111-116` unchanged; Dashboard showed the full 70,033-section dataset live). Native heap — the arena that holds materialized `ContentItem`s — went 48-62MB fresh → ~76MB on Learn → ~82MB with the reader → **peak ~90MB** with search, against a **512MB** limit: ~18% utilisation. Java/Dalvik heap stayed flat at 3-7MB. Guest attribution ran after **every** scenario across 5 app launches: clean every time — no `FATAL EXCEPTION`, `am_crash`, ANR, or `lmkd` kill of our package, ever. ⇒ The eager load demonstrably happens and demonstrably does not threaten the device. **Consequence: the pending Learn eager-load change is a performance improvement, NOT a P0 fix — do not ship it claiming it fixes a crash.** **The refutation rests on mechanism, not just samples:** `contentIndexProvider` loops `CurriculumId.all`, so **all 9 curricula — Talmud Bavli included — are materialized on the very first Learn open**, whatever the user then browses. Bavli's content was therefore already resident inside that ~90MB peak. **GAP NOW CLOSED (no residual):** Talmud Bavli was subsequently drilled deep — Learn → תלמוד בבלי → נזיקין → **בבא בתרא** (a 176-daf tractate) → reader, paged **34 dapim** (daf 2א → 36א) across 68 chevron taps. Device survived throughout. Memory: 62MB baseline → **session-wide peak 125,566 KB (~24% of the 512MB limit)** → GC pulled it back to ~110MB. (Quote the raw KB: ~123MB and ~125MB both circulate for this sample, being its MiB vs decimal readings.) So there *is* a real, mild **growth-then-plateau/oscillate** pattern in the reader under sustained paging (likely retained per-daf rendered state) — worth a look as a perf item, but it levels off rather than climbing monotonically, and its peak leaves ~76% headroom. **Largest curriculum, deepest drill, still REFUTED.** A second round using the host-side logcat recorder reproduced the same numbers (66-75MB native, 3-7MB Dalvik) and traced **one genuine guest-level SIGKILL** — which was correctly *excluded*, not waved off: no `lmkd` line named our package, ~40 unrelated processes were reaped with signal 9 in the same ~45s window just after `lmkd` initialized post-cold-boot on a 2GB AVD, and our app's own riverpod logs showed it still on **Dashboard** providers with `contentIndex` never touched and 53MB native. **Two real (non-crash) findings came out of this work:** (a) **reader next-chevron swallows input** — quantified across two independent rounds (5 taps → 1 advance; 68 taps → 34 advances, i.e. ~2 taps eaten per page): a genuine lost-input usability defect on the app's primary *reading* surface, filed P2 with repro steps; (b) **reader memory retention** under sustained paging (above) — a perf item, not a risk. |
