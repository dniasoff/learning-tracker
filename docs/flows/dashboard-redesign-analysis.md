...
> ⚠️ **Status — 2026-04-19:** Epic 20 (DNI-210) — the dashboard UI redesign this document specifies — was canceled in Linear on 2026-04-15 (all 12 child stories DNI-211..DNI-222). However, the **per-track data-model infrastructure** (trackId on config tables, track-scoped DAOs, per-track scheduler, `StudyDayConfigs` keyed by trackId) **did land** via Epic 23 (hard-tier auth) and Epic 21 (multi-account) work. The UI redesign itself was not built; the dashboard at `lib/features/dashboard/` is still the pre-redesign version. This document remains the authoritative design reference if Epic 20 is ever re-scoped and resumed.
