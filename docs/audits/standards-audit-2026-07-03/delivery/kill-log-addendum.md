...
- **Register verify status (audit time):** CONFIRMED, UPHELD 1/1, no refutations (`docs/_archive/superseded/standards-audit-2026-07-03-work/verify-chunks/grp-060.json`).
...
## AUD-scheduler-18 — Convert ScheduledUnit/StudyDayPattern/ProjectionResult in overdue_types.dart to @freezed instead of hand-rolled ==/hashCode

- **Wave:** parked
- **Severity:** P3
- **Delivery disposition:** `skipped-refuted` (merge-lane `w4r2c9`, 2026-07-17). Ledger row was previously `parked` ("roadmap section-4 uncited - parked per PO-approved scope 2026-07-03"); reclassified to `skipped-refuted` because the underlying question has since been affirmatively resolved by a later decision, not merely left deferred.
- **Basis — resolved by documented exemption, no code change required:** `AUD-scheduler-19` (merged — commits `ce999c80`/`f84ff8fa`, both ancestors of this branch's current `HEAD`) added a "Documented Judgment Calls" exemption to `docs/coding-standards.md` (L62–77, specifically L74) naming exactly this finding's three target classes — `ScheduledUnit`, `StudyDayPattern`, `ProjectionResult` in `lib/features/scheduler/domain/projection/overdue_types.dart` — citing the file's own zero-codegen design constraint (no `build_runner`/`@freezed` dependency permitted) as the reason hand-rolled equality is retained there deliberately, not by omission. This is enforced by `make audit` check 82/82 (`learning_tracker/Makefile:1136`), which greps `lib/features/scheduler/domain/` for hand-written `bool operator ==` outside the three documented exemption files and fails the gate on any new violation; currently green. Independently re-verified against the current tree: `grep -n "operator =="  learning_tracker/lib/features/scheduler/domain/projection/overdue_types.dart` shows exactly 3 hand-rolled overrides (lines 27, 66, 107), matching the exemption's own description. Converting them to `@freezed` now, as this finding recommends, would directly violate the documented, PO-approved decision rather than fix a live defect. No code change made.
- **Logged by:** merge-lane w4r2c9 agent (sonnet), 2026-07-17.

---

## AUD-scheduler-20 — Delete or properly @Deprecated the dead, self-documented-buggy toggleStudyDay provider

- **Wave:** parked
- **Severity:** P3
- **Delivery disposition:** `skipped-refuted` (merge-lane `w4r1c24`, 2026-07-19). Ledger row was previously `parked` ("roadmap section-4 uncited - parked per PO-approved scope 2026-07-03"); reclassified to `skipped-refuted` because the underlying target was affirmatively removed by ancestor commits, not merely left deferred.
- **Basis — already fixed in code, independently confirmed:** `toggleStudyDayProvider` was deleted outright by ancestor commit `4d46ca0f` (`fix(AUD-scheduler-06): delete dead StudyDayConfigRepositoryImpl/toggleStudyDay`); its coverage-test reference was separately deleted by ancestor commit `3ef9453a` (`fix(test): AUD-t-cross-15 - delete tautological generated-providers coverage test, add AG-3 checker`). Both commits are confirmed ancestors of this branch's current `HEAD`. Independently re-verified against the current tree: `grep -rn toggleStudyDay learning_tracker/lib/ learning_tracker/test/` returns 0 matches. Both ACs already satisfied; no code change made.
- **Logged by:** merge-lane w4r1c24 agent (sonnet), 2026-07-19.

---

## AUD-scheduler-21 — Delete or wire in the dead scheduler PaceIndicator/ProjectedCompletionText widget file

- **Wave:** parked
- **Severity:** P3
- **Delivery disposition:** `skipped-refuted` (merge-lane `w4r1c20`, 2026-07-19). Ledger row was previously `parked` ("roadmap section-4 uncited - parked per PO-approved scope 2026-07-03"); reclassified to `skipped-refuted` because the underlying target was affirmatively removed/consolidated by ancestor commits, not merely left deferred.
- **Basis — already fixed in code, independently confirmed:** ancestor commit `81d4d12a` (`fix(scheduler): AUD-scheduler-08 - delete dead pace_indicator.dart + test`) deleted the scheduler's duplicate `lib/features/scheduler/presentation/widgets/pace_indicator.dart` and its test outright; ancestor commit `a3619891` (`fix(AUD-repo-01): de-dupe 7 duplicate public type names, land AG-4 audit gate`) separately renamed the surviving progress-feature widget's class from `PaceIndicator` to `ProgressPaceIndicator`, which independently kills the AG-4 duplicate-name collision this finding also flags. Both commits are confirmed ancestors of this branch's current `HEAD`. Independently re-verified against the current tree: `grep -rn "class PaceIndicator" learning_tracker/lib/` returns 0 matches; `grep -rn "class ProgressPaceIndicator" learning_tracker/lib/` returns 1 match (`lib/features/progress/presentation/widgets/pace_indicator.dart:15`); `find learning_tracker -iname pace_indicator.dart` returns only the single surviving progress-feature file, no orphan. Both ACs ("Only one public `PaceIndicator` class exists in lib/, or the two are clearly distinctly named." and "No orphaned widget file with zero production call sites remains.") are satisfied. Sibling finding `AUD-scheduler-08` (targeting the same dead file) is independently already merged/fixed. No code change made.
- **Logged by:** merge-lane w4r1c20 agent (sonnet), 2026-07-19.

---

## AUD-scheduler-25 — Tag pace_calculator.dart's TODOs with real DNI-#### Linear ids

- **Wave:** parked
- **Severity:** P3
- **Delivery disposition:** `skipped-refuted` (merge-lane `w4r1c24`, 2026-07-19). Ledger row was previously `parked` ("roadmap section-4 uncited - parked per PO-approved scope 2026-07-03"); reclassified to `skipped-refuted` because the underlying target was affirmatively resolved by an ancestor commit, not merely left deferred.
- **Basis — already fixed in code, independently confirmed:** both target TODOs in `learning_tracker/lib/features/scheduler/domain/services/pace_calculator.dart` (lines 7 and 25) already carry `(DNI-393)`, added by ancestor commit `ddc2af09` (`fix(AUD-repo-02): give 7 untracked TODO/FIXME comments a Linear id, generalize AG-6 audit gate`) — `git blame` confirms both lines were introduced by that commit, and it remains an ancestor of this branch's current `HEAD`. That same commit also generalized `make audit` check 10/15 into the AG-6 grep this finding's AC#2 asks for (any `// TODO`/`// FIXME` in `lib/`, `test/`, `integration_test/` without a `DNI-<digits>` reference fails the gate). Independently re-verified against the current tree: `grep -n TODO learning_tracker/lib/features/scheduler/domain/services/pace_calculator.dart` shows both lines already suffixed `(DNI-393)`; re-ran the AG-6 checker's own grep pipeline (`grep -rnP '^\s*//\s*(TODO|FIXME)\b' lib/ test/ integration_test/ --include="*.dart" | grep -v "\.g\.dart" | grep -v "\.freezed\.dart" | grep -v "DNI-[0-9]" | grep -v "^lib/firebase_options.dart:"`) standalone from `learning_tracker/` — 0 matches repo-wide (exit 1, no output). Both ACs already satisfied; no code change made.
- **Logged by:** merge-lane w4r1c24 agent (sonnet), 2026-07-19.

---

## AUD-settings-12 — Route account_actions_sheet.dart's Sign Out / Delete Account icon glyphs through a theme token instead of a Colors.white literal

- **Wave:** parked
- **Severity:** P3
- **Delivery disposition:** `skipped-refuted` (merge-lane `w4r2c6`, 2026-07-19). Ledger row was previously `parked` ("roadmap section-4 uncited - parked per PO-approved scope 2026-07-03"); reclassified to `skipped-refuted` because the finding's own cited rule never fires on the literal it targets, not merely left deferred.
- **Basis — rule doesn't apply, independently confirmed:** the finding's cited checker, `no_color_literal_outside_theme` (`packages/custom_lints/lib/src/rules/no_color_literal_outside_theme.dart`), only flags `Color(0x...)` hex-literal constructor calls (`addInstanceCreationExpression`, `typeName == 'Color'`); it never flags `Colors.white`. The rule's own test suite documents this as intentional (`no_color_literal_outside_theme_test.dart`, "does not flag named Color constants"). This finding's own `verify` record in `findings.json` already carries this exact refutation from an earlier review round (upheld 1/2, chair-adjudicated) - the citation was factually wrong at the time and remains wrong now. `coding-standards.md`'s AUD-sacred_time-09 entry (2026-07-17) independently documents this same exclusion as deliberate and matching codebase-wide use of `Colors.white` (e.g. `account_actions_sheet.dart` lines 35, 97, 118 already use it untouched). Re-ran the rule's `testAnalyzeAndRun()` against `learning_tracker/lib/features/settings/presentation/widgets/account_actions_sheet.dart` on the current tree: zero hits. No code changed.
- **Logged by:** merge-lane w4r2c6 agent (sonnet), 2026-07-19.
